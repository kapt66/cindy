...
import {
  ...
  discoverSubagentDefinitions as discoverUnfilteredSubagentDefinitions,
} from '../subagent-definitions.js';

let root: string;
const directoryLinkType = process.platform === 'win32' ? 'junction' : 'dir';

async function discoverSubagentDefinitions(
  options: Parameters<typeof discoverUnfilteredSubagentDefinitions>[0],
) {
  const found = await discoverUnfilteredSubagentDefinitions(options);
  // Windows 的默认 %TEMP% 位于用户目录下。项目定义扫描会按产品语义一路向上查找，
  // 因而可能合法地发现真实的 ~/.claude/agents；测试只断言当前 fixture 创建的定义。
  return found.filter((definition) => {
    const relative = path.relative(root, definition.filePath);
