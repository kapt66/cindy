...
function installFakeHost(
  ...
  const host = {
    ...
    ensureStartedWithTimeout: vi.fn(
      async (_timeoutMs: number, _label: string) => ensureStarted(),
    ),
    request,
    subscribeThread,
    unsubscribeThread,
...
describe("CodexAgent permissions", () => {
  ...
  it("keeps a Host workflow gate observable in Full access", async () => {
    ...
    const host = installFakeHost(agent, (method) =>
      method === Method.TurnStart
        ? { turn: { id: "turn-host-workflow" } }
        : undefined,
    );
    const handle = await agent.startSession({
      sessionId: "session-host-workflow",
      model: "gpt-5.5",
      workingDir: "/repo",
      permissionMode: "bypassPermissions",
    });

    await handle.send({ type: "user", content: "edit a file" });
    const turnParams = host.request.mock.calls.find(
      ([method]) => method === Method.TurnStart,
    )?.[1] as Record<string, unknown>;
    expect(turnParams).toMatchObject({
    ...
    await expect(
      handlers.commandExecutionApproval({
        threadId: "start-thread-id",
        turnId: "turn-host-workflow",
        itemId: "cmd-host-workflow",
        command: "touch server.go",
        cwd: "/repo",
      }),
...
describe("CodexAgent MCP thread context hooks", () => {
  it("registers native Skill roots before thread start and partitions hosts by revision", async () => {
    ...
    for (const [index, revision] of ["revision-a", "revision-b"].entries()) {
      ...
      const threadIndex = requests.findIndex(
        (request) => request.method === Method.ThreadStart,
      );
      ...
      expect(requests[rootsIndex]!.params).toEqual({
        extraRoots: [
          path.join("/snapshots", revision, "claude-plugin", "skills"),
        ],
      });
    }
  ...
  it("fails startup when the native Skill root cannot be registered", async () => {
    ...
    await expect(
      ...
      /native Skill registration failed.*registration unavailable/i,
    );
    expect(
      createdTransports[0]!.lines.some((line) => line.includes("thread/start")),
    ).toBe(false);
    await agent.dispose();
  });
  ...
  it("runs utility calls on the shared local host instead of spawning another local host", async () => {
    ...
    createdTransports[0].setMockResponse(Method.AccountRateLimitsRead, {
      result: rateLimits,
    });
    createdTransports[0].setMockResponse(
  ...
  it("refuses account reset RPCs on a differently-authenticated active host", async () => {
    ...
    await expect(agent.readAccountRateLimits()).rejects.toThrow(
  ...
  it("isolates sessions that disable native subagents and passes the host policy to spawn config", async () => {
    const prepareCodexExtraSpawnConfig = vi.fn(async () => ({
      extraArgs: [],
      extraEnv: {},
    }));
    ...
    const ordinary = await agent.startSession({
      sessionId: "session-native-subagents-default",
      model: "gpt-5.4",
      workingDir: "/repo-a",
      nativeSkillPluginPath: "/snapshots/shared/claude-plugin",
      nativeSkillRevision: "shared-revision",
    });
    const restricted = await agent.startSession({
      sessionId: "session-native-subagents-disabled",
      model: "gpt-5.4",
      workingDir: "/repo-b",
      nativeSkillPluginPath: "/snapshots/shared/claude-plugin",
      nativeSkillRevision: "shared-revision",
    ...
    const restrictedThreadStart = createdTransports[1]!.lines
      .map(
        (line) =>
          JSON.parse(line) as {
            method?: string;
      ...
      .find((request) => request.method === Method.ThreadStart);
    expect(restrictedThreadStart?.params?.config).toMatchObject({
  ...
  it("accepts the combat environment MCP check without ordinary approval in Full access", async () => {
    const deps = createDeps(
      {},
      {
        getMcpToolApprovalPolicy: () => "prompt-each-time",
    ...
    deps.isHostToolExecutionPolicyActive = () => true;
    deps.evaluateHostToolExecution = vi.fn((context) =>
      context.input &&
      typeof context.input === "object" &&
      ["check_combat_environment", "list_tools"].includes(
        String((context.input as Record<string, unknown>).toolName ?? ""),
      )
        ? { behavior: "allow" as const }
        : { behavior: "deny" as const, reason: "unknown MCP tool" },
    );
    const agent = new CodexAgent(deps);
    const host = installFakeHost(agent);
    ...
    handlers.itemStarted?.({
      ...
      turnId: "turn-1",
      item: {
        id: "combat-environment-check",
        type: "mcpToolCall",
        server: "mcp_router",
        tool: "check_combat_environment",
        pluginId: null,
      },
    });
    ...
    expect(deps.evaluateHostToolExecution).toHaveBeenCalledWith(
      expect.objectContaining({
        toolName: "mcp:mcp_router",
        input: expect.objectContaining({
          toolName: "check_combat_environment",
        }),
        action: { kind: "mcp" },
      }),
    );
    ...
    expect(deps.evaluateHostToolExecution).toHaveBeenLastCalledWith(
      expect.objectContaining({
        input: expect.objectContaining({ toolName: "list_tools" }),
      }),
    );
    await handle.close();
  });

  it("does not let Full access bypass a Host workflow denial for an MCP call", async () => {
    const deps = createDeps(
      {},
      {
        getMcpToolApprovalPolicy: () => "prompt-each-time",
    ...
    deps.isHostToolExecutionPolicyActive = () => true;
    deps.evaluateHostToolExecution = vi.fn(() => ({
      behavior: "deny" as const,
      reason: "combat environment is not ready",
    }));
    const agent = new CodexAgent(deps);
    const host = installFakeHost(agent);
    const handle = await agent.startSession({
      sessionId: "session-host-denied-full-access-mcp",
      model: "gpt-5.4",
      workingDir: "/repo",
      permissionMode: "bypassPermissions",
    });
    ...
    const result = await handlers.mcpServerElicitation({
      ...
      _meta: {
        codex_approval_kind: "mcp_tool_call",
        tool_name: "check_combat_environment",
        tool_params: { name: "check_combat_environment", args: {} },
      },
      message: "Allow tool call",
      requestedSchema: {},
    });
  ...
  it("registers ask_user_question as a thread/start dynamic fallback for non-xAI Codex new threads", async () => {
    ...
    expect(startParams.dynamicTools?.[0]?.description).toContain(
      "Do not repeat the same question or confirmation",
    );
    expect(
