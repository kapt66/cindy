...
// Test events intentionally expose heterogeneous adapter payloads.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
type AgentEvent = Omit<CoreAgentEvent, "data"> & { data: any };
type LooseThreadEventHandlers = {
  // Test transport stores handlers with heterogeneous protocol signatures.
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  [K in keyof ThreadEventHandlers]?: (...args: any[]) => any;
};

const { MockCodexTransport, createdTransports, createdStdioOptions } =
  vi.hoisted(() => {
    ...
    class MockCodexTransport {
      ...
      static beforeThreadStartResponse:
        ((transport: MockCodexTransport) => Promise<void> | void) | null = null;
      ...
      private readonly responseOverrides = new Map<
        string,
        {
          result?: unknown;
      ...
      async writeLine(line: string): Promise<void> {
        ...
        const responseOverride = req.method
          ? this.responseOverrides.get(req.method)
          : undefined;
        if (responseOverride) {
        ...
        if (req.method === "thread/rollback") {
          this.emitLine({
            ...
            result: {
              thread: {
                id: `rollback-thread-${MockCodexTransport.threadSeq++}`,
              },
            },
          });
          return;
        ...
        if (req.method === "experimentalFeature/enablement/set") {
          const params = req.params as
            { enablement?: { memories?: unknown } } | undefined;
...
function installFakeHost(
  ...
  opts: {
    ...
    subagentRoute?: {
      ...
      catalogModel: string;
      reasoningEffort:
        import("./app-server/protocol.js").ReasoningEffort | null;
    };
  ...
  const subscribeThread = vi.fn(
    (_threadId: string, handlers: ThreadEventHandlers) => {
  ...
  const waitForMcpTool = vi.fn(
    async () =>
      opts.codexBrowserMcpToolAvailable ??
      opts.codexBrowserUseAvailable === true,
  );
  const getRemoteCompactionProviderId = vi.fn(
    () => opts.remoteCompactionProviderId ?? null,
  );
  const getSessionMcpConfig = vi.fn(
    (sessionInstanceId?: string) =>
  ...
  const getOpenAiWebSocketsEnabled = vi.fn(
    () => opts.openAiWebSocketsEnabled !== false,
  );
  const host = {
    ensureStarted,
    // startSession 的 initialize 直调走限时变体 (codex R13 P1): fake 里
    // 直接委托 ensureStarted (超时语义由 host.test.ts 的真 transport 覆盖)。
    ensureStartedWithTimeout: vi.fn(
      async (_timeoutMs: number, _label: string) => {
        void _timeoutMs;
        void _label;
...
describe("CodexAgent permissions", () => {
  it("graceful stop sends only turn/interrupt for the current turn", async () => {
    ...
    const host = installFakeHost(agent, (method) => {
      if (method === Method.TurnStart)
        return { turn: { id: "turn-graceful-stop" } };
  ...
  it("starts Review on an isolated memory-free host with an immutable read-only sandbox", async () => {
    const artifactDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "codex-review-artifact-"),
    );
    ...
    const host = installFakeHost(
      ...
      {
        buildSessionMcpConfig: () => ({
          "mcp_servers.should_not_exist": { command: "write" },
        }),
    ...
    expect(
      (threadStart.config as Record<string, unknown>)["skills.config"],
    ).toEqual([
      {
        path: "/home/test/.codex/plugins/cache/personal/unsafe-plugin/1.0.0/skills/review/SKILL.md",
        enabled: false,
    ...
    expect(threadStart.config).not.toHaveProperty([
      "mcp_servers.should_not_exist",
    ]);
    expect(threadStart.config).not.toHaveProperty([
      "mcp_servers.codex_apps.enabled",
    ]);
    expect(threadStart.config).not.toHaveProperty([
      "mcp_servers.configured_server.enabled",
    ]);
    ...
    await expect(
      handle.send({
        type: "user",
        content: [
          { type: "file", path: gitConfigPath, mimeType: "text/plain" },
        ],
      }),
    ).rejects.toThrow(/refused/i);
    expect(
      host.request.mock.calls.some(([method]) => method === Method.TurnStart),
    ).toBe(false);
    ...
    await handle.send({
      ...
      content: [
        {
          type: "text",
          text: "Review this Markdown, PDF, and image in one turn.",
        },
    ...
    const reviewText = (
      turnStart.input as Array<{ type?: string; text?: string }>
    ...
    if (!handle.setPermissionMode)
      throw new Error("expected permission control");
  ...
  it("fails closed when Review sees an unknown runtime-only MCP server", async () => {
    const reviewDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "codex-review-runtime-mcp-"),
    );
    ...
    await expect(
      ...
      /runtime MCP servers without transport-bearing config: unknown_runtime_mcp/,
    );
  });

  it("fails closed when a runtime MCP only matches a transportless plugin entry", async () => {
    const reviewDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "codex-review-plugin-mcp-"),
    );
    tempRoots.push(reviewDir);
    const agent = new CodexAgent(createDeps());
    installFakeHost(
  ...
  it("refuses to start Review when Codex native memory cannot be disabled", async () => {
    const reviewDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "codex-review-memory-"),
    );
    tempRoots.push(reviewDir);
    const agent = new CodexAgent(createDeps());
    installFakeHost(
  ...
  it("makes policy turns host-observable and only prompts for forced Auto actions", async () => {
    ...
    const resolver = vi.fn(
      async (_request: InteractionRequest): Promise<InteractionDecision> => {
        void _request;
        return {
    ...
    if (!handle.setPermissionMode)
      throw new Error("expected setPermissionMode");
...
describe("CodexAgent capability routing", () => {
  ...
  it("preserves the remote daemon Browser companion snapshot", async () => {
    ...
    expect([...hosts.values()][0]?.getCodexBrowserUseVersion()).toBe(
      "26.727.51351",
    );
  ...
  it("applies host-owned plugin policy to new and resumed Codex 0.145 threads", async () => {
    ...
    expect(startHost.request).toHaveBeenCalledWith(
      Method.SkillsList,
      {
        cwds: ["/repo"],
        forceReload: false,
    ...
    const cases = [
      {
        ...
        initialPrompt:
          "用 $feishu-delegate:message-feishu-coworkers 制定查询计划",
        plan: "1. 查询消息",
  ...
  it("does not unlock a downstream MCP when an explicit steering message is rejected", async () => {
    ...
    await vi.waitFor(() => {
      expect(
        host.request.mock.calls.filter(
          ([method]) => method === Method.TurnSteer,
        ),
      ).toHaveLength(1);
    });
    let elicitationSettled = false;
    const elicitationPromise = handlers
  ...
  it("records explicit capability selection after a late steer acknowledgement", async () => {
    ...
    try {
      ...
      const timeoutAssertion =
        expect(steerPromise).rejects.toThrow(/did not acknowledge/i);
  ...
  it("falls back to inherited thread selection when a child item predates turn binding", async () => {
    ...
    const host = installFakeHost(
      agent,
      (method) =>
        method === Method.TurnStart ? pendingTurnStart.promise : undefined,
      { userAgent: "mock-codex/0.145.0" },
    );
    const handle = await agent.startSession({
...
describe("CodexAgent reference directories", () => {
  ...
  it("keeps the thread-level reference profile active without repeating it on turn/start", async () => {
    ...
    await handle.send({
      type: "user",
      content: "use the replacement reference",
    });
    ...
    const [, noReferencesTurn] = turnCalls()[3] as [
      string,
      Record<string, unknown>,
    ];
  ...
  it("replaces an unused thread instead of resuming before its first rollout exists", async () => {
    ...
    const [, replacementStart] = startCalls[1] as [
      string,
      Record<string, unknown>,
    ];
    ...
    expect(replacementStart.config).toHaveProperty(
      `permissions.${profileName}`,
    );
    expect(replacementStart.developerInstructions).toBe(
      initialStart.developerInstructions,
    );
  ...
  it("preserves a Fast mode toggle made while replacing an unused thread", async () => {
    ...
    const host = installFakeHost(agent, (method) => {
      ...
      if (method === Method.TurnStart)
        return { turn: { id: "turn-fast-replacement" } };
      return undefined;
    });
    const handle = await agent.startSession({
  ...
  it("cancels a pending unused-thread replacement before turn/start", async () => {
    ...
    await expect(sendPromise).rejects.toThrow(
      /send cancelled before acceptance/i,
    );
    replacementGate.resolve({
      thread: { id: "start-thread-2" },
      model: "gpt-5.4",
  ...
  it("restores the thread-level profile before using references added after an earlier turn", async () => {
    ...
    const [, resumeParams] = resumeCalls[0] as [
      string,
      Record<string, unknown>,
    ];
    ...
    expect(turnParams.runtimeWorkspaceRoots).toEqual([
      "/repo",
      "/shared-added",
    ]);
    expect("permissions" in turnParams).toBe(false);
  ...
  it("restores the thread-level profile after Full access before the next reference turn", async () => {
    ...
    await handle.send({
      type: "user",
      content: "return to read-only references",
    });
  ...
  it("preserves a Fast mode toggle made while profile resume is pending", async () => {
    ...
    const sendPromise = handle.send({
      type: "user",
      content: "restore with fast mode",
    });
    await waitForExpectation(() => {
      expect(
        host.request.mock.calls.filter(
  ...
  it("reapplies roots and the profile when a stale daemon requires resume + retry", async () => {
    ...
    for (const [, params] of turnCalls as Array<
...
describe("CodexAgent.listCustomizations", () => {
  it("uses filesystem scanning without starting the Codex app-server", async () => {
    const root = await fs.mkdtemp(
      path.join(os.tmpdir(), "xdt-codex-customizations-"),
    );
    tempRoots.push(root);
    ...
    const agent = new CodexAgent(createDeps());
    const result = await agent.listCustomizations({
      workingDirs: [repo],
      kinds: ["skill"],
    });
...
describe("CodexAgent.refreshLocalModels", () => {
  it("uses an isolated OpenAI control-plane host without closing provider-oauth sessions", async () => {
    ...
    expect(
      createdTransports[0].lines.some(
        (line) =>
          (JSON.parse(line) as { method?: string }).method === Method.ModelList,
      ),
    ).toBe(false);
    expect(
      createdTransports[1].lines.some(
        (line) =>
          (JSON.parse(line) as { method?: string }).method === Method.ModelList,
      ),
    ).toBe(true);
  ...
  it("reads every model/list page and publishes one complete snapshot", async () => {
    ...
    (agent as unknown as { hosts: Map<string, unknown> }).hosts.set(
      "local",
      host,
    );
    ...
    expect(
      host.request.mock.calls.filter(([method]) => method === Method.ModelList),
    ).toEqual([
      [
        Method.ModelList,
    ...
    expect(onCodexLocalModelsListed).toHaveBeenCalledOnce();
    expect(
      onCodexLocalModelsListed.mock.calls[0][0].map(
  ...
  it("rejects a repeated cursor without publishing a partial snapshot", async () => {
    ...
    await expect(agent.refreshLocalModels()).rejects.toThrow(
  ...
  it("drops a late model/list result after the originating host is retired", async () => {
    ...
    const host = installFakeHost(agent, (method) => {
      if (method !== Method.ModelList) return undefined;
      (agent as unknown as { hosts: Map<string, unknown> }).hosts.delete(
        "local",
      );
  ...
  it("retires a wedged control-plane host after the model/list deadline", async () => {
    ...
    try {
      ...
      const agent = new CodexAgent(createDeps());
      const refresh = agent.refreshLocalModels({
        credentialMode: "oauth-bearer",
      });
...
describe("CodexAgent.startSession developerInstructions", () => {
  it("registers an immutable task Skill root before thread start", async () => {
    const agent = new CodexAgent(createDeps());
    const host = installFakeHost(agent, (method) => {
      if (method === Method.SkillsExtraRootsSet) return {};
      return undefined;
    });
    ...
    expect(methods.indexOf(Method.SkillsExtraRootsSet)).toBeGreaterThanOrEqual(
      0,
    );
    expect(methods.indexOf(Method.SkillsExtraRootsSet)).toBeLessThan(
      methods.indexOf(Method.ThreadStart),
    );
    expect(host.request).toHaveBeenCalledWith(
    ...
    expect(getHostOptions).toMatchObject({
      keyOverride: expect.stringContaining("revision-a"),
  ...
  it("rejects session creation when thread/resume fails", async () => {
    ...
    await expect(
      ...
      "Failed to resume Codex thread: Error: thread resume boom",
    );
  });
  ...
  it("passes the OpenAI compaction provider to thread/start only for oauth-family sessions", async () => {
    ...
    const oauthParams = host.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      modelProvider?: string;
    };
    ...
    const gatewayParams = host.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      modelProvider?: string;
    };
    ...
    const xaiParams = host.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      modelProvider?: string;
    };
  ...
  it("omits thread/start developerInstructions and registers prompt when codex proxy is active", async () => {
    ...
    const baselineAgent = new CodexAgent(
      createDeps(runtimeConfig, {
    ...
    const proxyAgent = new CodexAgent(
      createDeps(runtimeConfig, {
        registerCodexSystemPromptForThread,
        getGhostRosterPrompt: vi.fn(() => "GHOST ROSTER PROMPT"),
      }),
    );
    const proxyHost = installFakeHost(proxyAgent, undefined, {
      codexProxyActive: true,
    });
    ...
    const baselineParams = baselineHost.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      developerInstructions?: string;
    };
    const proxyParams = proxyHost.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      developerInstructions?: string;
    };
    ...
    expect(baselineParams.developerInstructions).toContain(
      "HOST PRODUCT PROMPT",
    );
    expect(baselineParams.developerInstructions).toContain(
      "GHOST ROSTER PROMPT",
  ...
  it("omits thread/resume developerInstructions and registers prompt when codex proxy is active", async () => {
    ...
    const startBaselineParams = startBaselineHost.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      developerInstructions?: string;
    };
    ...
    expect(startBaselineParams.developerInstructions).toContain(
      "HOST PRODUCT PROMPT",
    );
  ...
  it("restores developerInstructions on thread/resume when codex proxy is inactive and history lacks product prompt", async () => {
    ...
    const baselineHost = installFakeHost(baselineAgent);
    const registerCodexSystemPromptForThread = vi.fn();
    const resumeAgent = new CodexAgent(
      createDeps(runtimeConfig, {
        registerCodexSystemPromptForThread,
      }),
    );
    ...
    expect(resumeParams.developerInstructions).toBe(
      baselineParams.developerInstructions,
    );
    expect(resumeParams.developerInstructions).toContain("HOST PRODUCT PROMPT");
    expect(resumeParams.developerInstructions).toContain("USER PROMPT");
  ...
  it("keeps developerInstructions when codex proxy is active but no register hook exists", async () => {
    const agent = new CodexAgent(
      createDeps({ systemPrompt: "HOST PRODUCT PROMPT" }, {}),
    );
    const host = installFakeHost(agent);
  ...
  it("keeps thread/start developerInstructions identical to proxy resume registered text for the same prompt inputs", async () => {
    ...
    const registeredText =
      registerCodexSystemPromptForThread.mock.calls[0]?.[0]?.text;
...
describe("CodexAgent fast mode service tier", () => {
  it("normalizes app-server priority service tier from thread/start as fast mode", async () => {
    ...
    const turnStartParams = host.request.mock.calls.find(
      ([method]) => method === Method.TurnStart,
    )?.[1] as { serviceTier?: unknown } | undefined;
...
describe("CodexAgent thread/settings/update channel", () => {
  const settingsCalls = (host: {
    request: { mock: { calls: Array<[string, unknown]> } };
  }) =>
    host.request.mock.calls
      .filter(([m]) => m === Method.ThreadSettingsUpdate)
      .map(([, p]) => p);
  ...
  it("swallows thread/settings/update failure and still carries serviceTier via turn/start", async () => {
    ...
    const turn = host.request.mock.calls.find(
      ([m]) => m === Method.TurnStart,
    )?.[1] as { serviceTier?: unknown } | undefined;
...
describe("CodexAgent send", () => {
  ...
  it("clamps minimal effort set during a session to low before turn/start", async () => {
    ...
    if (!handle.setEffort)
      throw new Error("Codex handle should support setEffort");
  ...
  it("rejects accepted-or-rejected sends if the session closes before turn/start resolves", async () => {
    ...
    await expect(sendPromise).rejects.toThrow(
      /session is closed after turn\/start/i,
    );
  });
});
...
describe("CodexAgent MCP thread context hooks", () => {
  it("binds Browser companion availability to the concrete app-server host", async () => {
    ...
    const agent = new CodexAgent(
      createDeps({}, { prepareCodexExtraSpawnConfig }),
    );
  ...
  it("passes target context to Codex extra spawn config and reuses the shared local host", async () => {
    const prepareCodexExtraSpawnConfig = vi.fn(async () => ({
      extraArgs: [
        "-c",
        'mcp_servers.cindy_test.url="http://127.0.0.1:1234/mcp/cindy_test"',
      ],
    ...
    expect(createdTransports).toHaveLength(1);
    expect(
      createdTransports[0].lines.some((line) => line.includes("initialize")),
    ).toBe(true);
  ...
  it("keeps utility calls on the task host and account RPCs on an isolated control-plane host", async () => {
    ...
    MockCodexTransport.onCreate = (transport) => {
      transport.setMockResponse(Method.AccountRateLimitsRead, {
        result: rateLimits,
      });
    ...
    const pushCountAfterSet = createdTransports[0].lines.filter((line) =>
      line.includes("experimentalFeature/enablement/set"),
    ).length;

    await expect(
      agent.listAgentSkills({ workingDir: "/repo" }),
    ...
    await expect(
      agent.consumeAccountRateLimitResetCredit({
    ...
    expect(createdTransports).toHaveLength(2);
    expect(
      createdTransports[0].lines.some((line) => line.includes("skills/list")),
    ).toBe(true);
    expect(
      createdTransports[0].lines.some((line) => line.includes("config/read")),
    ).toBe(true);
    expect(
      createdTransports[0].lines.some((line) => line.includes("memory/reset")),
    ).toBe(true);
    expect(
      createdTransports[0].lines.some((line) =>
        line.includes("account/rateLimits/read"),
      ),
    ).toBe(false);
    expect(
      createdTransports[1].lines.some((line) =>
        line.includes("account/rateLimits/read"),
      ),
    ).toBe(true);
    expect(
    ...
    expect(
      createdTransports[0].lines.filter((line) =>
        line.includes("experimentalFeature/enablement/set"),
      ).length,
  ...
  it("does not retire an active local host for local credential restarts", async () => {
    ...
    await expect(agent.disposeLocalHostForCredentialChange()).rejects.toThrow(
      /active Codex session/i,
    );
    expect(createdTransports).toHaveLength(1);
    expect(createdTransports[0].closed).toBe(false);
  ...
  it("retires only the local host when local Codex credentials change", async () => {
    ...
    expect(
      createdTransports[1].lines.some((line) => line.includes("turn/start")),
    ).toBe(true);
  ...
  it("silently invalidates idle sessions when the local host is force-retired, while still notifying busy ones", async () => {
    ...
    const collectIdle = (async () => {
      for await (const event of idleHandle.events())
        idleEvents.push(event as unknown as Record<string, unknown>);
    })();
    const collectBusy = (async () => {
      for await (const event of busyHandle.events())
        busyEvents.push(event as unknown as Record<string, unknown>);
    })();
    ...
    await agent.forceDisposeLocalHostForAuthChange(
      "test forced retire with idle subscriber",
    );
  ...
  it("idle session send after force-retire rejects with stale-host error", async () => {
    ...
    await agent.forceDisposeLocalHostForAuthChange(
      "test forced retire idle then send",
    );
  ...
  it("collapses a pending turn/start session with terminal error + Done when the local host is force-retired", async () => {
    ...
    const sendPromise = handle.send({
      type: "user",
      content: "pending turn start",
    });
    await waitForExpectation(() => {
      expect(transport.lines.some((line) => line.includes("turn/start"))).toBe(
        true,
      );
    });

    await agent.forceDisposeLocalHostForAuthChange(
      "test forced retire with pending turn/start",
    );
  ...
  it("does not re-activate a force-retired session when a late turn/start success response arrives", async () => {
    ...
    await agent.forceDisposeLocalHostForAuthChange(
      "test forced retire with late start response",
    );
    ...
    if (req?.id !== undefined) {
      transport.emitMockLine({
        id: req.id,
        result: { turn: { id: "late-success-turn" } },
      });
    }
    await sendPromise.catch(() => undefined);
  ...
  it("keeps shared Codex sessions usable when ordinary stderr contains auth keywords", async () => {
    ...
    await expect(
      second.send({ type: "user", content: "still works" }),
    ...
    transport.emitMockLine({
      ...
      params: {
        threadId: "thread-1",
        turn: { id: "turn-to-steer-after-stderr" },
      },
    });
    await waitForExpectation(() => {
  ...
  it("serves XD gateway sessions on a live oauth superset host without restart", async () => {
    ...
    const oauthHandle = await agent.startSession({
      sessionId: "session-superset-oauth",
      providerId: "openai",
      model: "gpt-5.4",
      workingDir: "/repo-oauth",
    });
    const xdHandle = await agent.startSession({
      sessionId: "session-superset-xd",
      providerId: "xd",
      model: "codex/gpt-5.5",
      workingDir: "/repo-xd",
    });
  ...
  it("upgrades a cold gateway-key spawn to oauth when both credentials are available", async () => {
    ...
    const agent = new CodexAgent(
      createDeps({}, { auth, prepareCodexExtraSpawnConfig }),
    );

    const xdHandle = await agent.startSession({
      sessionId: "session-cold-xd-superset",
      providerId: "xd",
      model: "codex/gpt-5.5",
      workingDir: "/repo-xd",
    });
    const oauthHandle = await agent.startSession({
      sessionId: "session-cold-oauth-parallel",
      providerId: "openai",
      model: "gpt-5.4",
      workingDir: "/repo-oauth",
    });
  ...
  it("rejects a cold XD session without a gateway key even when OAuth is available", async () => {
    ...
    await expect(
      agent.startSession({
        sessionId: "session-cold-xd-without-key",
        providerId: "xd",
        model: "codex/gpt-5.5",
        workingDir: "/repo-xd",
      }),
  ...
  it("validates the gateway key before reusing a live oauth superset host", async () => {
    ...
    const oauthHandle = await agent.startSession({
      sessionId: "session-live-oauth-before-missing-key",
      providerId: "openai",
      model: "gpt-5.4",
      workingDir: "/repo-oauth",
    });
    ...
    await expect(
      agent.startSession({
        sessionId: "session-xd-missing-key-on-superset",
        providerId: "xd",
        model: "codex/gpt-5.5",
        workingDir: "/repo-xd",
      }),
  ...
  it("downgrades a cold superset spawn when the loopback proxy is unavailable", async () => {
    ...
    const handle = await agent.startSession({
      sessionId: "session-superset-downgrade",
      providerId: "xd",
      model: "codex/gpt-5.5",
      workingDir: "/repo-xd",
    });
  ...
  it("downgrades a cold superset spawn after a fatal OAuth proxy error", async () => {
    const prepareCodexExtraSpawnConfig = vi.fn(
      async (_providers: unknown, ctx: { credentialMode?: string }) => {
        if (ctx.credentialMode === "oauth-bearer") {
          const error = new Error("OAuth proxy is not ready");
          (error as { codexSpawnConfigFatal?: boolean }).codexSpawnConfigFatal =
            true;
          throw error;
        }
    ...
    const agent = new CodexAgent(
      createDeps(
        {},
        {
          auth,
          prepareCodexExtraSpawnConfig:
            prepareCodexExtraSpawnConfig as unknown as NonNullable<
              AgentDeps["prepareCodexExtraSpawnConfig"]
            >,
        },
      ),
    );
    const handle = await agent.startSession({
      sessionId: "session-superset-fatal-downgrade",
      providerId: "xd",
      model: "codex/gpt-5.5",
      workingDir: "/repo-xd",
    });
  ...
  it("rebuilds instead of reusing an oauth host when the proxy is inactive", async () => {
    ...
    const oauthHandle = await agent.startSession({
      sessionId: "session-nonproxy-oauth",
      providerId: "openai",
      model: "gpt-5.4",
      workingDir: "/repo-oauth",
    });
  ...
  it("deduplicates concurrent cold XD and OAuth starts while superset mode resolves", async () => {
    ...
    const xdStart = agent.startSession({
      sessionId: "session-concurrent-cold-xd",
      providerId: "xd",
      model: "codex/gpt-5.5",
      workingDir: "/repo-xd",
    });
    ...
    const oauthStart = agent.startSession({
      sessionId: "session-concurrent-cold-oauth",
      providerId: "openai",
      model: "gpt-5.4",
      workingDir: "/repo-oauth",
    });
  ...
  it("starts OAuth independently when a concurrent unresolved gateway inflight fails validation", async () => {
    ...
    const xdStart = agent.startSession({
      sessionId: "session-concurrent-invalid-xd",
      providerId: "xd",
      model: "codex/gpt-5.5",
      workingDir: "/repo-xd",
    });
    ...
    const oauthStart = agent.startSession({
      sessionId: "session-concurrent-valid-oauth",
      providerId: "openai",
      model: "gpt-5.4",
      workingDir: "/repo-oauth",
    });
  ...
  it("rejects an in-flight stale host when another credential mode replaces it before startup", async () => {
    ...
    const prepareCodexExtraSpawnConfig: NonNullable<
      AgentDeps["prepareCodexExtraSpawnConfig"]
  ...
  it("runs the credential mode switch coordinator before replacing an active local host", async () => {
    ...
    let keyHandle: Awaited<ReturnType<CodexAgent["startSession"]>> | null =
      null;
    const prepareCodexLocalCredentialModeSwitch: NonNullable<
      AgentDeps["prepareCodexLocalCredentialModeSwitch"]
    > = vi.fn(async () => {
  ...
  it("holds utility requests until a credential mode switch finishes", async () => {
    ...
    expect(createdTransports).toHaveLength(2);
    expect(
      createdTransports[1].lines.some((line) => line.includes("skills/list")),
    ).toBe(true);
  ...
  it("uses the host-owned remote credential mode instead of unrelated local OAuth state", async () => {
    ...
    const resolveRemoteCodexCredentialMode = vi.fn(
      () => "gateway-key" as const,
    );
    const agent = new CodexAgent(
      createDeps(
        {},
    ...
    expect(resolveRemoteCodexCredentialMode).toHaveBeenCalledWith(
      "mcpr:instance-1",
    );
  ...
  it("applies instance-bound MCP URLs to both thread/start and thread/resume", async () => {
    ...
    expect(startHost.getSessionMcpConfig).toHaveBeenCalledWith(
      "instance-start",
    );
    ...
    expect(resumeHost.getSessionMcpConfig).toHaveBeenCalledWith(
      "instance-resume",
    );
  ...
  it("retires the captured host and rejects an unused-thread replacement when release fails", async () => {
    ...
    const retireHostKey = vi
      .spyOn(
        agent as unknown as {
          retireHostKey: (...args: unknown[]) => Promise<void>;
        },
        "retireHostKey",
      )
  ...
  describe("websocket body recovery auto-retry", () => {
    ...
    function installRecoveryHost(
      agent: CodexAgent,
    ): ReturnType<typeof installFakeHost> {
      let turnSeq = 0;
    ...
    it.each([false, true])(
      ...
      async (startedBeforeCompletion) => {
        ...
        await waitForExpectation(() => expect(turnStarts).toBe(1));
        const handlers = host.getThreadHandlers();
        if (!handlers?.turnCompleted || !handlers.turnStarted)
          throw new Error("expected handlers");
    ...
    it("recovers from a failed turn/completed when no error notification was delivered", async () => {
      ...
      await handle.send({ type: "user", content: "hello" });
      const handlers = host.getThreadHandlers();
      if (!handlers?.turnCompleted)
        throw new Error("expected turnCompleted handler");
      handlers.turnCompleted({
        threadId: "start-thread-id",
        turn: {
          id: "turn-1",
  ...
  describe("overload auto-retry", () => {
    ...
    it("stops after the retry budget and only then reports a terminal error", async () => {
      ...
      try {
        ...
        const handlers = host.getThreadHandlers();
        if (!handlers?.error || !handlers.turnStarted)
          throw new Error("expected handlers");
        const events: AgentEvent[] = [];
        void (async () => {
          for await (const event of handle.events()) events.push(event);
      ...
      try {
        ...
        expect(
          events.some(
            (e) =>
              e.type === "error" &&
              (e.data as { isTerminal?: boolean; reason?: string })
                .isTerminal === true &&
              (e.data as { reason?: string }).reason ===
                "codex-turn-replay-retry-aborted",
          ),
        ).toBe(true);
        expect(
          events.some(
    ...
    it("resets the output guard on a new send even if turnStarted has not arrived yet", async () => {
      ...
      try {
        ...
        const handlers = host.getThreadHandlers();
        if (!handlers?.error) throw new Error("expected error handler");
        if (!handlers.itemStarted)
          throw new Error("expected itemStarted handler");
        if (!handlers.turnCompleted)
          throw new Error("expected turnCompleted handler");
        handlers.itemStarted({
          turnId: "turn-1",
    ...
    it("does not settle twice when Stop is followed by the start RPC rejecting", async () => {
      ...
      try {
        ...
        expect(
          String((terminal[0].data as { message?: string }).message),
        ).toContain("stopped");
    ...
    it("ignores an unattributable id-less capacity error instead of settling the new send", async () => {
      ...
      try {
        ...
        await vi.advanceTimersByTimeAsync(0);
        expect(
          events.slice(before).filter((e) => e.type === "error"),
        ).toHaveLength(0);
    ...
    it("does not let a stopped orphan turn buffered output block the next send retry", async () => {
      ...
      try {
        ...
        expect(
          added.filter((e) => e.type === "error" && e.data.isTerminal),
        ).toHaveLength(0);
        // 透出的是非终止的重试进度横幅。
        expect(
          added.filter((e) => e.type === "error" && !e.data.isTerminal).length,
        ).toBeGreaterThan(0);
    ...
    it("does not treat buffered control events as produced output when adopting a dead turn", async () => {
      ...
      try {
        ...
        const handlers = host.getThreadHandlers();
        if (!handlers?.error || !handlers.itemStarted)
          throw new Error("expected handlers");
        const events: AgentEvent[] = [];
        void (async () => {
          for await (const event of handle.events()) events.push(event);
        ...
        expect(turnStarts).toBe(2);
        expect(
          events.filter((e) => e.type === "error" && e.data.isTerminal),
        ).toHaveLength(0);
    ...
    it("keeps the foreign-terminal guard when the retry already activated its turn", async () => {
      ...
      try {
        ...
        await vi.advanceTimersByTimeAsync(0);
        expect(
          events.slice(before).filter((e) => e.type === "done"),
        ).toHaveLength(0);
        expect(handle.getCurrentTurnId?.()).toBe("turn-retry");
        expect(handle.isTurnRunning?.()).toBe(true);
    ...
    it("stays busy and settles on cancel while a capacity failure is only deferred", async () => {
      ...
      try {
        ...
        expect(
          after.some((e) => e.type === "status" && e.data?.isRunning === false),
        ).toBe(true);
        expect(handle.isTurnRunning?.()).toBe(false);
    ...
    it("interrupts an already-started retry turn when only the send signal aborts", async () => {
      ...
      try {
        ...
        const handlers = host.getThreadHandlers();
        if (!handlers?.error) throw new Error("expected error handler");
        if (!handlers.turnStarted)
          throw new Error("expected turnStarted handler");
        handlers.error({
          threadId: "start-thread-id",
          turnId: "turn-1",
          willRetry: false,
    ...
    it("does not count the SDK userMessage echo as produced output", async () => {
      ...
      try {
        ...
        const handlers = host.getThreadHandlers();
        if (!handlers?.error) throw new Error("expected error handler");
        if (!handlers.itemCompleted)
          throw new Error("expected itemCompleted handler");
        // 只有用户输入的回声, 模型一个字都没出。
        handlers.itemCompleted({
          turnId: "turn-1",
          item: { id: "item-echo", type: "userMessage", text: "hello" },
    ...
    it("settles the logical turn when only the send signal is aborted mid-backoff", async () => {
      ...
      try {
        ...
        expect(
          after.filter(
            (e) => e.type === "status" && e.data?.isRunning === false,
          ),
        ).not.toHaveLength(0);
    ...
    it("takes over an id-less capacity failure that lands while the initial turn/start is pending", async () => {
      ...
      try {
        ...
        expect(surfaced).toHaveLength(1);
        expect(surfaced[0].data).toMatchObject({
          isTerminal: false,
          willRetry: true,
        });
  ...
  it("keeps third-party routes on the user protocol and reviews with the current session model", async () => {
    const reviewAutoPermissionAction: ReturnType<
      typeof vi.fn<AutoReviewDelegate>
    > = vi.fn(async () => ({ verdict: "allow" as const }));
    const agent = new CodexAgent(
      createDeps({}, { reviewAutoPermissionAction }),
    );
    const host = installFakeHost(
      agent,
      (method) => {
    ...
    await handle.send({
      type: "user",
      content: "Check this project for type errors",
    });
    const turnParams = host.request.mock.calls.find(
      ([method]) => method === Method.TurnStart,
    )?.[1] as {
    ...
    const handlers = host.getThreadHandlers();
    if (!handlers?.commandExecutionApproval)
      throw new Error("expected commandExecutionApproval");
    await expect(
      handlers.commandExecutionApproval({
        threadId: "start-thread-id",
    ...
    expect(reviewAutoPermissionAction).toHaveBeenCalledWith(
      expect.objectContaining({
        agentKind: "codex",
        providerId: "xd",
    ...
    expect(reviewAutoPermissionAction.mock.calls[0]?.[0]).not.toHaveProperty(
      "transcript",
    );
    expect(reviewAutoPermissionAction.mock.calls[0]?.[0]).not.toHaveProperty(
      "tools",
  ...
  it("opens the approval UI only when the lightweight reviewer explicitly returns ask", async () => {
    const reviewAutoPermissionAction = vi.fn<AutoReviewDelegate>(
      async (_request) => {
        void _request;
        return {
    ...
    expect(resolver).toHaveBeenCalledOnce();
    const request = (
      resolver.mock.calls as unknown as Array<[InteractionRequest]>
    )[0]?.[0];
  ...
  it.each([
    ...
    "does not treat a missing confirmation as a user rejection after auto-review fails (%s)",
    async (reason) => {
      const reviewAutoPermissionAction = vi.fn<AutoReviewDelegate>(
        async () => null,
      );
      ...
      expect(
        notices.some((message) =>
          message.includes(`[${AUTO_REVIEW_CONFIRM_UNDELIVERED_CODE}]`),
        ),
      ).toBe(true);
      expect(
        notices.some((message) => message.includes("not a user rejection")),
      ).toBe(true);
      await handle.close();
    },
  );
  ...
  it.each(["close", "setPermissionMode"] as const)(
    "does not treat a system-dismissed confirmation as a user rejection after auto-review fails (%s)",
    async (action) => {
      const reviewAutoPermissionAction = vi.fn<AutoReviewDelegate>(
        async () => null,
      );
  ...
  it("asks the user when a file-change approval omits grantRoot", async () => {
    ...
    if (!handlers?.fileChangeApproval)
      throw new Error("expected fileChangeApproval handler");
    const resolver = vi.fn(async (): Promise<InteractionDecision> => ({
      kind: "permission",
      behavior: "allow",
    }));
  ...
  it("re-checks Ask and Full access after an in-flight Cindy auto-review", async () => {
    ...
    const askAgent = new CodexAgent(
      createDeps({}, { reviewAutoPermissionAction: askReviewer }),
    );
    const askHost = installFakeHost(askAgent);
    const askHandle = await askAgent.startSession({
      sessionId: "session-auto-review-late-ask",
      model: "gpt-5.5",
      providerId: "openai",
      workingDir: "/repo",
      permissionMode: "auto",
    ...
    if (!askHandlers?.commandExecutionApproval)
      throw new Error("expected commandExecutionApproval handler");
    ...
    const pendingAsk = askHandlers.commandExecutionApproval({
      threadId: "start-thread-id",
      turnId: "turn-late-ask",
      itemId: "cmd-late-ask",
      approvalId: "a-late-ask",
      command: "rm -rf build",
      cwd: "/repo",
    });
    ...
    expect(askResolver.mock.calls[0]?.[0]).toMatchObject({
      kind: "permission",
      suggestions: undefined,
    });
    ...
    const fullAgent = new CodexAgent(
      createDeps({}, { reviewAutoPermissionAction: fullReviewer }),
    );
    const fullHost = installFakeHost(fullAgent);
    const fullHandle = await fullAgent.startSession({
      sessionId: "session-auto-review-late-full",
      model: "gpt-5.5",
      providerId: "openai",
      workingDir: "/repo",
      permissionMode: "auto",
    ...
    if (!fullHandlers?.commandExecutionApproval)
      throw new Error("expected commandExecutionApproval handler");
    const fullResolver = vi.fn(
      async (_request: InteractionRequest): Promise<InteractionDecision> => {
        void _request;
        return {
          kind: "permission",
    ...
    const pendingFull = fullHandlers.commandExecutionApproval({
      threadId: "start-thread-id",
      turnId: "turn-late-full",
      itemId: "cmd-late-full",
      approvalId: "a-late-full",
      command: "rm -rf build",
      cwd: "/repo",
    });
  ...
  it("policy turn (unattended Auto): accepts only auto-approve, declines both prompt verdicts", async () => {
    ...
    const host = installFakeHost(agent, (method) => {
      if (method === Method.TurnStart)
        return { turn: { id: "turn-policy-danger" } };
      return undefined;
    });
    const handle = await agent.startSession({
    ...
    handle.setInteractionResolver(resolver);
    await handle.send(
      { type: "user", content: "do maintenance" },
      { turnPermissionPolicy: policy },
    );
  ...
  it("policy turn (unattended Auto): declines permission escalation that has no classifiable action (fail-closed)", async () => {
    ...
    const host = installFakeHost(agent, (method) => {
      if (method === Method.TurnStart)
        return { turn: { id: "turn-policy-perm" } };
      return undefined;
    });
    const handle = await agent.startSession({
    ...
    if (!handlers?.permissionsApproval)
      throw new Error("expected permissionsApproval handler");
  ...
  it("keeps Auto and switches to the current-model fallback when Guardian times out", async () => {
    ...
    const host = installFakeHost(agent, (method) => {
      if (method === Method.TurnStart)
        return { turn: { id: "turn-after-guardian-timeout" } };
      return undefined;
    });
    const handle = await agent.startSession({
  ...
  it("treats Guardian reviewer failures as unavailable without emitting a user-facing error", async () => {
    ...
    handlers.autoApprovalReviewCompleted({
      ...
      review: {
        ...
        rationale:
          "Automatic approval review failed: invalid reviewer response",
      },
      action: {
        type: "command",
        source: "shell",
    ...
    await handle.send({
      type: "user",
      content: "continue after reviewer failure",
    });
    const turnParams = host.request.mock.calls.find(
      ([method]) => method === Method.TurnStart,
    )?.[1] as {
  ...
  it("interrupts the running turn when tightening from Full access to ask", async () => {
    ...
    const interruptCall = host.request.mock.calls.find(
      ([method]) => method === Method.TurnInterrupt,
    );
  ...
  it("does not reactivate a turn whose completion arrived before the turn/start response", async () => {
    ...
    handlers.turnStarted?.({
      threadId: "start-thread-id",
      turn: { id: "turn-tombstone" },
    });
    handlers.turnCompleted?.({
      threadId: "start-thread-id",
      turn: { id: "turn-tombstone", status: "interrupted" },
    });
    ...
    await expect(
      handle.steer({ type: "user", content: "probe" }),
  ...
  it("registers ask_user_question as a thread/start dynamic fallback for non-xAI Codex new threads", async () => {
    ...
    expect(startParams.dynamicTools?.[0]?.description).toContain(
      "the user asks to choose",
    );
    expect(startParams.dynamicTools?.[0]?.description).toContain(
      "provide a generic list",
    );
    expect(startParams.dynamicTools?.[0]?.description).toContain(
      "Ask 1 to 3 short questions in a single call",
    );
    expect(startParams.dynamicTools?.[0]?.description).toContain(
      "returns the awaited result as a JSON string",
    );
    expect(startParams.dynamicTools?.[0]?.description).toContain(
      "JSON.parse(raw)",
    );
    expect(startParams.dynamicTools?.[0]?.description).toContain(
      "Do not read .content or .structuredContent",
    );
    expect(
      startParams.dynamicTools?.[0]?.inputSchema?.properties?.questions
        ?.description,
    ).toContain("Bundle independent choices");
    ...
    const openAiParams = openAiHost.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      dynamicTools?: Array<{
        type?: string;
    ...
    const xdParams = xdHost.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      dynamicTools?: Array<{
        type?: string;
  ...
  it("registers eager host dynamic tools and restores their handler on resume", async () => {
    ...
    const startAgent = new CodexAgent(
      createDeps(
        {},
        {
  ...
  it("keeps host dynamic tool calls behind the existing MCP approval policy", async () => {
    ...
    expect(result).toEqual({
      contentItems: [
        { type: "inputText", text: "The user declined this tool call." },
      ],
      success: false,
    });
    expect(presentation).toHaveBeenCalledWith({
  ...
  it("does not register ask_user_question dynamic fallback for xAI Codex providers", async () => {
    ...
    const explicitStartParams = explicitHost.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      dynamicTools?: unknown;
    };
    ...
    const implicitStartParams = implicitHost.request.mock.calls.find(
      ([method]) => method === Method.ThreadStart,
    )?.[1] as {
      dynamicTools?: unknown;
    };
  ...
  it("routes native requestUserInput without tool context to ask_user_question", async () => {
    ...
    if (!handlers?.requestUserInput)
      throw new Error("expected requestUserInput handler");
    ...
    handle.setInteractionResolver(async (req) => {
      ...
      return {
        kind: "ask_user_question",
        answers: { "Which mode should Codex use?": "Fast" },
      };
    });
  ...
  it("classifies requestUserInput from MCP tool calls as permission", async () => {
    ...
    const handlers = host.getThreadHandlers();
    if (!handlers?.itemStarted || !handlers.requestUserInput)
      throw new Error("expected handlers");
  ...
  it("applies a host shell-command denial before Full access auto-approval", async () => {
    ...
    const agent = new CodexAgent(
      createDeps({}, { getShellCommandPolicy: policy, logger }),
    );
    const host = installFakeHost(agent);
    const handle = await agent.startSession({
      ...
      workingDir: "/repo",
      permissionMode: "bypassPermissions",
      vendorOptions: { mekaCombatTargetSkillIdState: "missing" },
    });
    const handlers = host.getThreadHandlers();
    if (!handlers?.commandExecutionApproval)
      throw new Error("expected commandExecutionApproval handler");
    ...
    expect(warn).toHaveBeenCalledWith(
      "command execution denied by host policy",
      {
        requestId: "cmd-1",
        reason: "use the embedded iOS Simulator",
      },
  ...
  it("keeps an approval-path policy denial as the terminal turn reason", async () => {
    ...
    const agent = new CodexAgent(
      createDeps({}, { getShellCommandPolicy: policy }),
    );
    const host = installFakeHost(agent, (method) =>
    ...
    if (
      ...
      throw new Error(
        "expected commandExecutionApproval, itemCompleted and turnCompleted handlers",
      );
    }
    const events: AgentEvent[] = [];
    const collectEvents = (async () => {
    ...
    const terminal = events.filter(
      (event) =>
        ...
        (event as { data?: { isTerminal?: boolean } }).data?.isTerminal ===
          true,
    );
  ...
  it("keeps all declined approval items attached to an interrupted turn", async () => {
    const policy = vi.fn(({ command }: { command: string }) => ({
      decision: "deny" as const,
      reason: command.startsWith("open ")
        ? "first policy reason"
    ...
    ).resolves.toEqual([{ decision: "decline" }, { decision: "decline" }]);
  ...
  it("keeps an approval denial through late updates from a pre-existing parallel item", async () => {
    ...
    if (
      ...
      throw new Error(
        "expected approval, item lifecycle and turn diff handlers",
      );
    }
    const events: AgentEvent[] = [];
    const collectEvents = (async () => {
  ...
  it("keeps an approval denial through an approval item that was already pending", async () => {
    ...
    if (
      ...
      throw new Error(
        "expected approval, item lifecycle and turn completion handlers",
      );
    }
    const events: AgentEvent[] = [];
    const collectEvents = (async () => {
    ...
    await waitForExpectation(() =>
      expect(interactionResolver).toHaveBeenCalledTimes(1),
    );
  ...
  it("clears an approval denial when a new native plan item continues the turn", async () => {
    ...
    const host = installFakeHost(agent, (method) =>
      method === Method.TurnStart
        ? { turn: { id: "turn-plan-after-denial" } }
        : undefined,
    );
    const handle = await agent.startSession({
    ...
    if (
      ...
      throw new Error(
        "expected approval, itemStarted and turnCompleted handlers",
      );
    }
    const events: AgentEvent[] = [];
    const collectEvents = (async () => {
    ...
    expect(JSON.stringify(terminal)).not.toContain(
      "use the embedded iOS Simulator",
    );
  });
  ...
  it("lets an approval-path denial recover to a normal completed turn", async () => {
    ...
    if (!handlers?.commandExecutionApproval || !handlers.turnCompleted) {
      throw new Error(
        "expected commandExecutionApproval and turnCompleted handlers",
      );
    }
    const events: AgentEvent[] = [];
    const collectEvents = (async () => {
    ...
    await collectEvents;
    expect(
      events.filter((event) => (event as { type?: string }).type === "error"),
    ).toEqual([
      expect.objectContaining({
        data: expect.objectContaining({
          message: "use the embedded iOS Simulator",
  ...
  it("clears an approval denial once a replacement command continues the turn", async () => {
    ...
    expect(JSON.stringify(terminalErrors)).not.toContain(
      "use the embedded iOS Simulator",
    );
  });
  ...
  it("interrupts an already-started shell bypass and reports the policy reason", async () => {
    ...
    await expect(nextEvent(iterator)).rejects.toThrow(
  ...
  it("keeps explicit user Stop attribution after a policy hit and late blocked item", async () => {
    ...
    await handle.send({
      type: "user",
      content: "test user Stop after host policy",
    });
    expect(await nextEvent(iterator)).toMatchObject({
      type: "status",
      data: { isRunning: true },
  ...
  it("ends an active plan cycle after a host-policy interruption", async () => {
    ...
    const turnStartRequests = () =>
      host.request.mock.calls.filter(([method]) => method === Method.TurnStart);
    const [, planParams] = turnStartRequests()[0] as [
      string,
      Record<string, unknown>,
    ];
    ...
    const [, normalParams] = turnStartRequests()[1] as [
      string,
      Record<string, unknown>,
    ];
  ...
  it("keeps the turn visible when a host-policy interrupt cannot be acknowledged", async () => {
    ...
    await handle.send({
      type: "user",
      content: "test failed host-policy interrupt",
    });
    expect(await nextEvent(iterator)).toMatchObject({
      type: "status",
      data: { isRunning: true },
    ...
    await vi.waitFor(() => {
      expect(error).toHaveBeenCalledWith(
        "host policy could not interrupt running command",
        {
          turnId: "turn-1",
        },
      );
  ...
  it("excludes a server-resolved approval wait from the completed turn duration", async () => {
    ...
    try {
      ...
      expect(
        (done?.data as { usage?: { durationMs?: number } }).usage?.durationMs,
      ).toBe(3_000);
  ...
  it("preserves submitted answers from different concurrent user input prompts", async () => {
    ...
    const firstResultPromise = callQuestion(
      "req-first",
      "call-first",
      "first",
      "First question?",
    );
    const secondResultPromise = callQuestion(
      "req-second",
      "call-second",
      "second",
      "Second question?",
    );
    if (!firstResultPromise || !secondResultPromise)
      throw new Error("expected dynamic tool results");
    ...
    const firstReplay = await callQuestion(
      "req-first-replay",
      "call-first-replay",
      "first",
      "First question?",
    );
    const secondReplay = await callQuestion(
      "req-second-replay",
      "call-second-replay",
      "second",
      "Second question?",
    ...
    expect(firstReplay?.contentItems).toEqual([
      {
        type: "inputText",
        text: JSON.stringify({ first: { answers: ["First answer"] } }),
      },
    ]);
    expect(secondReplay?.contentItems).toEqual([
      {
        type: "inputText",
        text: JSON.stringify({ second: { answers: ["Second answer"] } }),
      },
    ]);
    expect(requestCount).toBe(2);
    await handle.close();
  ...
  it("cancels pending user input when serverRequest/resolved arrives", async () => {
    ...
    const iterator = handle.events()[Symbol.asyncIterator]();
    const handlers = host.getThreadHandlers();
    if (!handlers?.requestUserInput || !handlers.serverRequestResolved)
      throw new Error("expected handlers");
  ...
  it.each([
    ...
    "wakes joined user-input requests during $lifecycle cleanup",
    async ({ lifecycle, reason, trigger }) => {
...
describe("CodexAgent steer", () => {
  it("rejects when the session is already closed before steering", async () => {
    ...
    await expect(
      handle.steer({ type: "user", content: "steer message" }),
  ...
  it("injects the message into the active turn via turn/steer without interrupting", async () => {
    ...
    const steerCall = host.request.mock.calls.find(
      ([method]) => method === Method.TurnSteer,
    );
    ...
    expect((steerCall?.[1] as { input?: unknown[] }).input).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: "text", text: "steer message" }),
      ]),
    );
    expect(
      host.request.mock.calls.filter(
  ...
  it("propagates a server-side no-active-turn rejection for the normal-send fallback", async () => {
    ...
    const host = installFakeHost(agent, (method) => {
      if (method === Method.TurnSteer) {
        throw Object.assign(
          new Error(
            "codex app-server turn/steer error -32602: no active turn to steer",
          ),
          { code: -32602 },
        );
      }
  ...
  it("resolves as delivered when the turn ends before the steer acknowledgement arrives", async () => {
    ...
    for (let i = 0; i < 5; i += 1) {
      if (
        host.request.mock.calls.some(([method]) => method === Method.TurnSteer)
      )
        break;
      await Promise.resolve();
    }
...
describe("CodexAgent resume preparation", () => {
  ...
  it("does not call thread/resume when the host identifies an unsafe rollout", async () => {
    const prepareCodexResumeSession = vi.fn(async () => {
      throw new CodexResumePreparationBlockedError(
        "rollout may still have a live writer",
      );
    });
...
describe("CodexAgent.forkSdkSession", () => {
  it("prepares an imported source thread before thread/fork", async () => {
    ...
    expect(prepareCodexResumeSession).toHaveBeenCalledWith(
      "imported-source-thread",
    );
  ...
  it("forks the source thread, then rolls back the forked thread by tail turns", async () => {
    ...
    expect(host.unsubscribeThread).toHaveBeenNthCalledWith(
      2,
      "rollback-thread-id",
    );
  ...
  it("retires the ephemeral fork host and rejects when a forked child cannot be unloaded", async () => {
    const agent = new CodexAgent(createDeps());
    const host = installFakeHost(agent);
    host.unsubscribeThread.mockRejectedValueOnce(
  ...
  it("omits excludeTurns for daemons older than 0.145.0", async () => {
    ...
    const forkParams = host.request.mock.calls.find(
      ([m]) => m === Method.ThreadFork,
    )?.[1] as { excludeTurns?: boolean } | undefined;
  ...
  it("forks from a temporary rollout copy without unsafe payload lines", async () => {
    ...
    const codexHome = await fs.mkdtemp(
      path.join(os.tmpdir(), "xdt-codex-home-"),
    );
    try {
      ...
      const sourceRollout = path.join(
        sessionsDir,
        "rollout-2026-06-11-source-thread-id.jsonl",
      );
      await fs.writeFile(
        sourceRollout,
        `${[
          ...
          JSON.stringify({
            payload: { type: "reasoning", encrypted_content: "gAAA" },
          }),
          JSON.stringify({
            type: "event_msg",
            payload: {
  ...
  it("uses the rollout path returned by imported-thread preparation when stripping", async () => {
    const externalHome = await fs.mkdtemp(
      path.join(os.tmpdir(), "xdt-codex-external-"),
    );
    const sourceRollout = path.join(
      externalHome,
      "rollout-imported-thread.jsonl",
    );
    await fs.writeFile(
      sourceRollout,
      [
    ...
    try {
      ...
      const agent = new CodexAgent(
        createDeps({}, { prepareCodexResumeSession }),
      );
      let copied = "";
      const host = installFakeHost(agent, async (method, params) => {
        if (method === Method.ThreadFork) {
      ...
      (agent as unknown as { codexHome: string }).codexHome = path.join(
        externalHome,
        "empty-home",
      );
      await agent.forkSdkSession({
...
describe("CodexAgent rewind", () => {
  it("previews as conversation-only and commits via thread/rollback", async () => {
    ...
    const commitRewindFiles = handle.commitRewindFiles;
    if (!previewRewindFiles || !commitRewindFiles)
      throw new Error("expected rewind methods");
    ...
    const commitResult = await commitRewindFiles("", "", {
      tailTurnsToDrop: 2,
    });
  ...
  it("discards the replacement thread when close races with rollback cleanup", async () => {
    ...
    await expect(rewindPromise).resolves.toEqual({
...
describe("CodexAgent turn lifecycle", () => {
  it("escalates a persistent willRetry retry-loop to a terminal error (issue #677)", async () => {
    ...
    const terminalData = terminalError.data as {
      message: string;
      isTerminal: boolean;
    };
    ...
    expect(terminalData.message).toContain(
      "Route agent traffic via local proxy",
    );
  ...
  it("ignores an orphan turnStarted after a failed turn/start and interrupts it server-side (greptile P1)", async () => {
    ...
    const host = installFakeHost(agent, (method) => {
      if (method === Method.TurnStart) {
        if (failTurnStart) {
          throw new Error(
            "codex app-server turn/start timed out after 60000ms",
          );
        }
  ...
  it("clears an accepted started-before-resp turn when turn/start later fails (greptile R6 P1)", async () => {
    ...
    turnStart.reject(
      new Error("codex app-server turn/start timed out after 60000ms"),
    );
    await sendPromise;
  ...
  it("buffers an ambiguous turnStarted during a retry-pending window and interrupts it once the response proves it an orphan (codex R9 P2)", async () => {
    ...
    expect(
      host.request.mock.calls.some(
        ([method, params]) =>
          method === Method.TurnInterrupt &&
          (params as { turnId?: string }).turnId === "orphan-turn",
      ),
    ).toBe(false);
  ...
  it("discards buffered terminal events once the response proves the turn an orphan (greptile R11 P1)", async () => {
    ...
    handlers.turnCompleted?.({
      threadId: "start-thread-id",
      turn: { id: "orphan-turn", status: "failed" },
    });
    // 孤儿终态全在缓存里, 事件流无输出。
    await expect(nextEvent(iterator)).rejects.toThrow(
      "timed out waiting for event",
  ...
  it("registers buffered spawn lineage before slow turn reconciliation and replays child terminal state", async () => {
    ...
    expect(events.some((event) => event.type === "agent_task_update")).toBe(
      false,
    );
  ...
  it("quarantines idle orphan turn events after a failed turn/start (codex R15 P1)", async () => {
    ...
    handlers.turnCompleted?.({
      threadId: "start-thread-id",
      turn: { id: "orphan-turn", status: "completed" },
    });
    await expect(nextEvent(iterator)).rejects.toThrow(
      "timed out waiting for event",
    );
  ...
  it("keeps verified descendant server requests outside the root orphan gate", async () => {
    ...
    const iterator = handle.events()[Symbol.asyncIterator]();
    const resolver = vi.fn(
      async (request: InteractionRequest): Promise<InteractionDecision> => {
        if (request.kind === "permission") {
          return { kind: "permission", behavior: "allow" };
        }
    ...
    await expect(
      handlers.dynamicToolCall(
        ...
        { requestId: "child-dynamic-request" },
      ),
    ).resolves.toMatchObject({
      success: true,
    });
  ...
  it("does not escalate rate-limit backoff retries to a terminal backend-unreachable error (codex R12 P2)", async () => {
    ...
    const rateLimited = {
      ...
      error: {
        message:
          "unexpected status 429 Too Many Requests, url: https://chatgpt.com/backend-api/codex/responses",
      },
    } as const;
  ...
  it("REPRO: reconstructs a late completed-only collab tool call", async () => {
    ...
    expect(reconstructedToolUse.backgroundTurnStartedAt).toEqual(
      expect.any(Number),
    );
    expect(reconstructedToolUse.backgroundTurnStartedAt).toBeGreaterThanOrEqual(
      turnStartedWindowStart,
    );
    expect(reconstructedToolUse.backgroundTurnStartedAt).toBeLessThanOrEqual(
      turnStartedWindowEnd,
    );
  ...
  it("REPRO: scopes descendant updates from a completed parent turn as background", async () => {
    ...
    expect(lateDescendantUpdate.backgroundTurnStartedAt).toEqual(
      expect.any(Number),
    );
  ...
  it("REPRO: reasserts the live running state for a late V1 spawn completion", async () => {
    ...
    expect(
      replayedRunningUpdate.backgroundTurnStartedAt,
    ).toBeGreaterThanOrEqual(parentTurnStartedWindowStart);
...
describe("CodexAgent plan mode", () => {
  ...
  async function startPlanSession(
    agent: CodexAgent,
    host: ReturnType<typeof installFakeHost>,
    sessionId: string,
  ) {
    const handle = await agent.startSession({
      sessionId,
  ...
  it("carries collaborationMode plan on turn/start and omits it for normal sessions", async () => {
    ...
    const handle = await startPlanSession(
      agent,
      host,
      "session-plan-turn-params",
    );

    await handle.send({ type: "user", content: "make a plan" });

    const [, params] = turnStartCalls(host)[0] as [
      string,
      Record<string, unknown>,
    ];
    ...
    const [, normalParams] = turnStartCalls(normalHost)[0] as [
      string,
      Record<string, unknown>,
    ];
  ...
  it("dispatches plan_review after a plan turn and starts the implementation turn on approval", async () => {
    ...
    const [, implParams] = turnStartCalls(host)[1] as [
      string,
      {
  ...
  it("requests the default marker again when turn/start retries after a daemon restart", async () => {
    ...
    const [, failedImplParams] = turnStartCalls(host)[1] as [
      string,
      {
        collaborationMode?: {
          mode: string;
    ...
    const [, retryParams] = turnStartCalls(host)[2] as [
      string,
      {
        collaborationMode?: {
          mode: string;
    ...
    expect(failedImplParams.collaborationMode).toEqual({
      mode: "default",
      settings: PLAN_SETTINGS,
    });
    expect(retryParams.collaborationMode).toEqual({
      mode: "default",
      settings: PLAN_SETTINGS,
    });
    await handle.close();
  ...
  it("emits a terminal event when the approved plan implementation turn cannot start", async () => {
    ...
    const host = installFakeHost(agent, (method) => {
      if (method === Method.TurnStart) {
        ...
        if (turnSeq === 2) {
          throw new Error(
            "Codex send cannot be accepted: stale host before turn/start",
          );
        }
        return { turn: { id: `turn-${turnSeq}` } };
      }
    ...
    const handle = await startPlanSession(
      agent,
      host,
      "session-plan-approve-start-fails",
    );
    const iterator = handle.events()[Symbol.asyncIterator]();
    handle.setInteractionResolver(async () => ({
      kind: "plan_review",
  ...
  it("appends the revised plan to the implementation message when user edited it", async () => {
    ...
    expect(implParams.input).toEqual([
      {
        type: "text",
        text: "Implement the plan. Follow this revised plan:\n\n1. do X\n2. do Z (revised)",
      },
    ]);
    await handle.close();
  });
  ...
  it("stays in plan mode and sends the feedback as a revision turn on deny with reason", async () => {
    ...
    const [, reviseParams] = turnStartCalls(host)[1] as [
      string,
      {
        input: Array<{ type: string; text?: string }>;
    ...
    expect(reviseParams.input).toEqual([
      { type: "text", text: "再加一步测试" },
    ]);
    // 修订 turn 仍在本轮 plan 循环内(collaborationMode 保持 plan),
    // 但一次性勾选已在首次 send 消耗。
    expect(reviseParams.collaborationMode).toEqual({
      mode: "plan",
      settings: PLAN_SETTINGS,
    });
    expect(handle.getPlanMode?.()).toBe(false);
  ...
  it("emits a terminal event and ends the plan cycle when revision profile restoration fails", async () => {
    ...
    for (let i = 0; i < 30 && !sawTerminalError; i++) {
      ...
      if (ev.type === "error") {
        expect(ev.data).toMatchObject({
          message: expect.stringContaining(
            "Failed to restore Codex read-only reference permissions",
          ),
          isTerminal: true,
        });
        sawTerminalError = true;
    ...
    await handle.send({ type: "user", content: "continue normally" });
    const [, nextParams] = turnStartCalls(host)[1] as [
      string,
      Record<string, unknown>,
    ];
    expect(
      (nextParams.collaborationMode as { mode?: string } | undefined)?.mode,
    ).not.toBe("plan");
    await handle.close();
  });
  ...
  it("does not attach a prior turn plan to a later task_complete", async () => {
    ...
    while ((await nextEvent(iterator)).type !== "done") {
      /* drain first turn */
    }
    ...
    for (
      let i = 0;
      i < 20 && secondTurnEvents.at(-1)?.type !== "done";
      i += 1
    ) {
      const event = await nextEvent(iterator);
      secondTurnEvents.push(event);
    }
    expect(secondTurnEvents.some((event) => event.type === "tool_use")).toBe(
      false,
    );
    await handle.close();
  });
  ...
  it("does not intercept native plan items from an active normal turn after arming the next turn", async () => {
    ...
    handlers.itemCompleted?.({
      ...
      item: {
        type: "plan",
        id: "turn-1-plan",
        text: "normal turn update_plan content",
      },
    } as never);
    handlers.turnCompleted?.({
      threadId: "start-thread-id",
  ...
  it("does not start a revision turn for listenerless plan review fallback", async () => {
    ...
    const handle = await startPlanSession(
      agent,
      host,
      "session-plan-listenerless-dismiss",
    );
    handle.setInteractionResolver(async () => ({
      kind: "plan_review",
      behavior: "deny",
  ...
  it("plan mode is one-shot: the next send after an empty plan cycle resets to default", async () => {
    ...
    const [, planParams] = turnStartCalls(host)[0] as [
      string,
      Record<string, unknown>,
    ];
    expect(planParams.collaborationMode).toEqual({
      mode: "plan",
      settings: PLAN_SETTINGS,
    });
    expect(handle.getPlanMode?.()).toBe(false);
    ...
    const [, resetParams] = turnStartCalls(host)[1] as [
      string,
      Record<string, unknown>,
    ];
  ...
  it("does not mark a failed plan turn/start as touched plan history", async () => {
    ...
    await handle.send({ type: "user", content: "just do it" });
    const [, params] = turnStartCalls(host)[1] as [
      string,
      Record<string, unknown>,
    ];
  ...
  it("honors the per-send plan intent snapshot over the current armed state", async () => {
    ...
    await handle.send(
      { type: "user", content: "queued plan request" },
      { planMode: true },
    );
    const [, planParams] = turnStartCalls(host)[0] as [
      string,
    ...
    await handle.setPlanMode?.(true);
    await handle.send(
      { type: "user", content: "queued normal message" },
      { planMode: false },
    );
    const [, normalParams] = turnStartCalls(host)[1] as [
      string,
      Record<string, unknown>,
    ];
    expect(
      (normalParams.collaborationMode as { mode?: string } | undefined)?.mode,
    ).not.toBe("plan");
    expect(handle.getPlanMode?.()).toBe(true);
    await handle.close();
  });
  ...
  it("honors explicit normal sends while a plan review is pending", async () => {
    ...
    const handle = await startPlanSession(
      agent,
      host,
      "session-plan-review-control-send",
    );
    const pendingDecision = deferred<InteractionDecision>();
    const seen: unknown[] = [];
    handle.setInteractionResolver(async (req) => {
    ...
    await handle.send(
      { type: "user", content: "control message" },
      { planMode: false },
    );
    const [, controlParams] = turnStartCalls(host)[1] as [
      string,
      Record<string, unknown>,
    ];
    ...
    handlers.itemCompleted?.({
      ...
      item: {
        type: "plan",
        id: "turn-2-plan",
        text: "control turn native plan item",
      },
    } as never);
    handlers.turnCompleted?.({
      threadId: "start-thread-id",
  ...
  it("keys early native plan item interception to the current turn send intent", async () => {
      ...
      if (turnSeq === 2) {
        handlers.itemCompleted?.({
          ...
          item: {
            type: "plan",
            id: `${turnId}-plan`,
            text: "control turn early native plan item",
          },
        } as never);
        handlers.turnCompleted?.({
          threadId: "start-thread-id",
    ...
    const handle = await startPlanSession(
      agent,
      host,
      "session-plan-early-control-send",
    );
    const iterator = handle.events()[Symbol.asyncIterator]();
    const pendingDecision = deferred<InteractionDecision>();
    const seen: unknown[] = [];
    ...
    for (let i = 0; i < 30 && !sawControlPlanItem; i++) {
      ...
      if (
        data.toolName === "update_plan" &&
        data.input?.text === "control turn early native plan item"
      ) {
...
describe("CodexAgent upstream-response-idle watchdog", () => {
  ...
  function collectEvents(
    handle: Awaited<ReturnType<CodexAgent["startSession"]>>,
  ): AgentEvent[] {
  ...
  it("turn 开始后上游持续静默 → 推终态 error 并 interrupt turn", async () => {
    ...
    try {
      ...
      const terminal = seen.find(
        (ev) =>
          ev.type === "error" &&
          (ev.data as { reason?: string } | null)?.reason ===
            "upstream_response_idle_timeout",
      );
      expect(terminal).toBeDefined();
      expect((terminal!.data as { isTerminal?: boolean }).isTerminal).toBe(
        true,
      );
      expect(
        host.request.mock.calls.some(
  ...
  it("graceful stop interrupt 被拒绝后重新武装同一 turn 的 idle watchdog", async () => {
    ...
    try {
      ...
      const host = installFakeHost(agent, (method) => {
        if (method === Method.TurnStart)
          return { turn: { id: "turn-stop-rejected" } };
        if (method === Method.TurnInterrupt) {
          interruptAttempts += 1;
          if (interruptAttempts === 1)
            return Promise.reject(new Error("interrupt rejected"));
          return {};
        }
        return undefined;
      });
      const handle = await startIdleSession(
        agent,
        "session-stop-rejected-watchdog",
      );
      const seen = collectEvents(handle);
      await handle.send({ type: "user", content: "go" });
      const handlers = host.getThreadHandlers();
  ...
  it("graceful stop 确认超时后重新武装同一 turn 的 idle watchdog", async () => {
    ...
    try {
      ...
      const host = installFakeHost(agent, (method) => {
        if (method === Method.TurnStart)
          return { turn: { id: "turn-stop-timeout" } };
        if (method === Method.TurnInterrupt) {
          interruptAttempts += 1;
          if (interruptAttempts === 1)
  ...
  it("graceful stop 失败期间的新 provider 活动保留新 deadline,不恢复旧快照", async () => {
    ...
    try {
      ...
      const host = installFakeHost(agent, (method) => {
        if (method === Method.TurnStart)
          return { turn: { id: "turn-stop-new-activity" } };
        if (method === Method.TurnInterrupt) {
          interruptAttempts += 1;
  ...
  it("迟到的旧 turn collab 收口不重启新 turn idle 计时或注入前台状态", async () => {
    ...
    try {
      ...
      const handle = await startIdleSession(
        agent,
        "session-idle-late-collab-terminal",
      );
      const seen = collectEvents(handle);
...
describe("CodexAgent reconnect-stall watchdog", () => {
  ...
  it("graceful stop interrupt 被拒绝后重新武装同一 turn 的 reconnect watchdog", async () => {
    ...
    try {
      ...
      const host = installFakeHost(agent, (method) => {
        if (method === Method.TurnStart)
          return { turn: { id: "turn-reconnect-stop-rejected" } };
        if (method === Method.TurnInterrupt) {
          interruptAttempts += 1;
          if (interruptAttempts === 1)
            return Promise.reject(new Error("interrupt rejected"));
  ...
  it("graceful stop 确认超时后恢复 reconnect watchdog 的剩余 deadline", async () => {
    ...
    try {
      ...
      const host = installFakeHost(agent, (method) => {
        if (method === Method.TurnStart)
          return { turn: { id: "turn-reconnect-stop-timeout" } };
        if (method === Method.TurnInterrupt) {
          interruptAttempts += 1;
          if (interruptAttempts === 1)
            return new Promise<never>(() => undefined);
  ...
  it("子代理进展不算主 turn 恢复:reconnect deadline 照旧到点收口", async () => {
    ...
    try {
      ...
      expect(
        seen.filter((e) => e.type === "agent_task_update").length,
      ).toBeGreaterThan(0);
  ...
  it("迟到 completed 证明旧 turn 已结束时，即使两次 interrupt 都 reject 也不退役共享 host", async () => {
    ...
    try {
      ...
      const host = installFakeHost(agent, (method) => {
        ...
        if (method === Method.TurnInterrupt) {
          interruptCall += 1;
          return interruptCall === 1
            ? firstInterrupt.promise
            : secondInterrupt.promise;
        }
        return undefined;
      });
      const handle = await agent.startSession({
...
describe("CodexAgent context window reporting", () => {
  ...
  function agentWithWindows(windows: Record<string, number>) {
    return agentWithVerified(
      (_providerId, modelId) => windows[modelId] ?? null,
    );
  }
  ...
  it("解析结果随目录刷新即时生效,不被会话启动时缓存住", async () => {
    let live = 200_000;
    const agent = agentWithVerified((_p, modelId) =>
      modelId === GATEWAY_MODEL ? live : null,
    );
    const host = installFakeHost(agent);
    const handle = await agent.startSession({
  ...
  it("活跃 turn 的用量按该 turn 的模型归属,不受中途 setModel 影响", async () => {
    const agent = agentWithWindows({
      [GATEWAY_MODEL]: 372_000,
      [WIDE_MODEL]: 1_000_000,
    });
    const host = installTurnCapableHost(agent);
    const handle = await agent.startSession({
  ...
  it("projects the locked V1 proxy route instead of the inherited parent model", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect(
        updates.some((update) => update.model === "deepseek/deepseek-v4-pro"),
      ).toBe(false);
    });
  ...
  it("forwards root and descendant Codex diffs into one merged turn capture event", async () => {
    ...
    handlers.descendantThreadStarted({
      thread: {
        id: "grandchild-diff-thread",
        parentThreadId: "child-diff-thread",
      },
    });
    ...
    const lastDiff = events
      .filter((event) => event.type === "turn_diff")
      .at(-1);
    expect(lastDiff?.data).toMatchObject({
      turnId: "turn-root-diff",
      cwd: "/repo",
    });
    expect((lastDiff?.data as { diff: string }).diff).toContain("root.txt");
    expect((lastDiff?.data as { diff: string }).diff).toContain("child.txt");
    expect((lastDiff?.data as { diff: string }).diff).toContain(
      "grandchild.txt",
    );
  ...
  it("ignores descendant diffs that arrive after the child turn is terminal", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect((last?.data as { diff?: string } | undefined)?.diff).toContain(
        "+child-final",
      );
    });
    handlers.turnCompleted?.({
  ...
  it("merges same-file descendant hunks without duplicating the diff block", async () => {
    ...
    const merged = (
      events.filter((event) => event.type === "turn_diff").at(-1)?.data as {
        diff: string;
      }
    ).diff;
  ...
  it("composes overlapping descendant hunks into the final same-file patch", async () => {
    ...
    await vi.waitFor(() => {
      ...
      const data = last?.data as
        { diff?: string; isComplete?: boolean } | undefined;
    ...
    expect(
      applyPatch(
        applied as string,
        formatPatch(reversePatch(parsePatch(merged))),
      ),
    ).toBe(original);
  ...
  it("composes overlapping descendant hunks with no-newline metadata", async () => {
    ...
    handlers.turnStarted?.({
      threadId: "start-thread-id",
      turn: { id: "turn-overlap-no-newline" },
    });
    handlers.turnDiffUpdated?.({
      threadId: "start-thread-id",
    ...
    handlers.descendantThreadStarted({
      thread: {
        id: "child-overlap-no-newline",
        parentThreadId: "start-thread-id",
      },
    });
    handlers.descendantNotification(
      "child-overlap-no-newline",
      "turn/diff/updated",
      {
  ...
  it("composes an overlapping pure deletion at the preimage line", async () => {
    ...
    await vi.waitFor(() => {
      const last = events.filter((event) => event.type === "turn_diff").at(-1);
      expect(
        (last?.data as { isComplete?: boolean } | undefined)?.isComplete,
      ).toBe(true);
    });
    const merged = (
      events.filter((event) => event.type === "turn_diff").at(-1)?.data as {
  ...
  it("clears an empty descendant snapshot from the merged turn diff", async () => {
    ...
    await vi.waitFor(() => {
      ...
      expect((last?.data as { diff?: string } | undefined)?.diff).not.toContain(
        "child.txt",
      );
    });
    handlers.turnDiffUpdated?.({
  ...
  it("completes the subagent card without any thread/started (codex 0.145 real behavior)", async () => {
    ...
    await vi.waitFor(() => {
      const spawned = events
        ...
        .map(
          (event) =>
            event.data as { taskId?: string; status?: string; model?: string },
        )
...
describe("CodexAgent compaction storm escalation", () => {
  ...
  it("开着 Maker Memory 时同样熔断(改成无条件回调没打断 flush 路径)", async () => {
    ...
    const agent = new CodexAgent(
      createDeps({ makerMemoryEnabled: true }, {
        logger: spyLogger,
        makerMemory: {
          async getStore() {
            return {
