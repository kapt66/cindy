...
“未绑定战斗工作流”描述成 P4、UnityMCP 或 MCPRouter 不可用，也不得因此结束回合。三项依赖的
...
启动回执不得要求首条用户可见消息先回显角色身份或 P4、UnityMCP、MCPR 三项状态。用户请求
...
`safety-boundaries`，并显式启用业务层的 `project-agent`；平台层的
