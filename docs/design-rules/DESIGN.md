...
- **Session-loading wordmark sheen — narrow functional exception (Desktop approved 2026-08-10).** `BrandLoadingMark` may use one neutral `transparent -> --text-primary -> transparent` gradient solely as a mask-clipped highlight inside the official Light/Dark wordmark while a session's message tree is deferred. It appears only after a 200ms delayed-reveal threshold and is not a reusable background, separator, fill, or surface treatment. Motion and brand-surface constraints are registered in §14.4 and §15.7.
...
- UI 段：{10, 11, 12, 13, 14, 15, 16}px；标题 / 内容段：{18, 20, 24, 28}px。开发者写死的档位不得低于 10px。
- 组件使用 `tailwind.config.ts` 提供的 `text-<n>` token 类；禁止新增 `text-[Npx]` 任意值。四个事实源必须同步：本节、`tailwind.config.ts`、`globals.css` 的 `--text-<n>` 默认值，以及 `useFontSettings.ts` 的运行时缩放表。
- 与可缩放字号 token 共用的行高必须使用无单位比例，不得使用固定 px 行高，否则用户放大 UI 字号时会裁切文字。
...
#### Session-loading wordmark sheen

This is the sixth sanctioned motion class and is limited to the session-switch deferred-loading overlay only. `BrandLoadingMark` may reveal after a 200ms delayed-reveal threshold, then move one neutral mask-clipped highlight across the official wordmark using compositor-only transform animation. It unmounts when the message tree mounts, pauses while the app is hidden, and becomes static under reduced motion. The standard `animate-spinner` remains the default loader everywhere else.
...
- **品牌红 `#DF0C27` 只用于字标红元素等品牌 accent，跨模式不变**；**禁止作页面背景**（wave4 改判，见 `token-decision-table §3` 对 `#df0c27` 的语义判定），不渗入面板内部（呼应 §15.10 红色边界）。画布底走 `--login-bg-base`（亮 `#EDEDED` / 深 `#1F1F1E`）。错误红 `#D91F37` 同样跨模式不变（语义豁免，呼应 §10 豁免族）。
...
| `LoginTitleBlock` | 双 | 标题 + 副标题 | `title`, `subtitle`, `regionPill?`（桌面兼容） | 标题 y=31 h=38 32 Bold `--login-title-text`；副标题 @(70,75) 540 宽 ≤2 行顶对齐 20 Regular `--login-secondary-text`（2026-07-24 拍板，原 599×23 单行作废） |
| `LoginRealmSelector` | 桌面 | CN / Global 登录服务区 | `value`, `disabled`, `onChange` | 对齐会话草稿 `VendorSegmentedSwitcher`：面板设计坐标 top=106、h60（缩放后 30px），外层与分段均 r9999 胶囊；轨道 padding 6、分段 gap 4（缩放后 3px / 2px），每项左右 padding 28、字号 28（缩放后 14px），首屏输入框单独下移至 y=180，槽位互不重叠。容器 `--surface-chip`，选中项 `--surface-elevated` + `--border-default`，未选项透明，使用 `--focus-ring-soft` 焦点环；选中态不得使用品牌红 |
...
**登录服务区选择器（Desktop）**：CN / Global 是可操作的二选一控件，不是构建身份徽标。选择器显示服务区本地化名称，切换后同步更新登录方式；企业 SSO home-realm 发现不得反向改写该选择。Mobile 暂无此控件。
