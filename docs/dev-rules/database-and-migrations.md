...
- **即使 migration 已进入当前产品主分支，只要安装包尚未包含它，也不得写进正式 profile。**
  unpackaged writer 在 CindyMeka / CindyMekaDev 上发现 pending migration 就 fail closed；
  append-only 只保证向前升级，不保证旧二进制向前兼容。要验证新 schema 必须使用
...
- **新客户端写 `messages` 的连接必须先注册 `cjk_seg`。** SQLite 自定义函数是连接级的，
  不进 schema。0103 起持久 `messages_fts` insert/update 触发器只写原文，并在
