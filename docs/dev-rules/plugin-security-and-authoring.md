...
| 能力实现（网络／通知／确认／文件系统／技能／宿主等） | `networkSlot.ts`、`notifySlot.ts`、`badgeSlot.ts`（未读角标，落盘账本 `ghostUnreadStore.ts`）、`confirmSlot.ts`（往返桥 `ghostConfirmDialogBridge.ts`，renderer 落地 `cindy-brain/GhostConfirmDialogHost.tsx`）、`fsSlot.ts`、`cindySlot.ts`、`skillSlot.ts`、`agentSlot.ts`、`errandSlot.ts`（派活执行链在 `maker-ipc/ghostErrandRunner.ts`，每插件配置在 `errandPrefsStore.ts`）、`iosSimulatorSlot.ts`（当前台前任务的公开状态与 Host viewer 入口）；持久作品库见 [`plugin-library-storage.md`](plugin-library-storage.md)，主实现在 `libraryVault.ts`、`librarySlot.ts`、`libraryDbCore.ts` |
| 面板供片、注入主题 token 与协议                      | `apps/desktop/src/renderer/cindy-brain/ghostPanelTheme.ts`、`cindy-ghost://` 分支 |
| 装入与更新入口 UI                                    | `apps/desktop/src/renderer/cindy-brain/installFlow.tsx`（安装不设能力确认层，能力说明见下一行） |
...
| Meka MCPRouter 能力                                  | [`mcpr-plugin-capability.md`](mcpr-plugin-capability.md) 与 MCPRouter 对端契约 |
...
- **启停态是授权口径里唯一的不对称例外**：`.disabled` 镜像读取时只往停用方向合并
  （`enabled = receipt.enabled && !镜像存在`）——停用必须永远能成功，状态根不可写时镜像
  是 `setEnabled(false)` 唯一还能落笔的地方；镜像**不能**把插件往启用方向翻。撤销陈旧批准
  且无法落盘时退回**进程内隔离**，由读批准状态的**唯一入口**统一投影成 `invalid`，并同时
  熄灯运行中的实例；下一轮启动对账成功即自愈。
...
- **Forge 的源码区与 Host 受管根互斥。** `ghost_forge_scaffold` / `ghost_forge_pack` /
  `ghost_forge_install` 的目标必须是独立作者目录，不得是安装根或批准状态根；命中时
  `ghost_forge_pack` 返回 `SOURCE_IS_INSTALLED_PLUGIN`，并按 realpath **双向**挡住大小写折叠与
  软链／junction 别名。会话工作目录内直接放行；工作目录外走与
  `ghost_call` 过户相同的会话权限路径（本地 Full Access 自动放行，Auto 交当前会话
  AI 审阅，Ask 弹确认卡，远程／缺会话／查询失败 fail closed），禁止在 Host 已放行
  后再因目录边界悄悄硬断。Host 放行跨出 workdir 时必须同时记下当次授权的规范目录与授权
  代次，并在长打包之后、写 `.cindy` 之前复验代次；授权已失效即拒绝，不得把放行事实当成
  一次永久许可。
  Meka 开发目录是同一门禁下的独立通道：Agent 传入的 `channel` 仅是业务元数据，
  `channel: "meka"` 本身**不**授予会话外源码根；只有 Host 托管的 `MekaDevPluginManager`
  可在当前 data owner 的开发注册表明确授权后，直接调用 Forge 打包实现并传入该条目对应的
  规范化源码根，且登记根自身不得作为打包目标。`ghost_forge_pack` 只负责校验与打包；只有用户明确
  要求后调用独立的
  `ghost_forge_install` 才安装或更新，不因 scaffold／pack 成功而隐式安装。理由不是洁癖：
  在已安装目录里就地制作"更新包"会让版本与权限 diff 以被改过的现场为基线，把未经确认的
  manifest 送进运行时授权。
...
- Agent 按逻辑插件 ID 查找或调用插件时，Host 必须先匹配正式安装插件的精确 ID；只有当前
  data owner 的开发注册表存在该逻辑 ID 到 runtime ID 的明确记录时，才回落到对应开发副本。
  禁止通过重新计算或猜测 `meka-dev-*` ID 探测开发插件，避免跨 owner 复用陈旧身份或让开发
  副本遮蔽正式安装版本。
...
- 开发目录检查、首次登记、自动同步和本地／远端打包不依赖活动任务。Main 以用户通过目录
  ...
  状态根和随包 seed 根的禁区列表；这项适配只属于 Meka 开发目录，**不是** Agent
  `ghost_forge_pack` 跨出会话 workdir 的放行依据（Agent 侧只能走第 3.1 节的会话权限路径），
  也不得允许把 Host 受管根或其祖先登记为开发源码。
...
- 开发插件卡片按 manifest 能力显示直接动作：只有声明 `panel` 才显示“打开界面”，只有
  ...
  打包、打开界面、使用、更新和管理等卡内独立动作必须阻止点击事件继续冒泡到整卡详情
  入口，一次点击只能触发用户选中的动作；回归测试须同时断言动作回调执行且详情回调未执行。
...
- `ios-simulator` 槽（v3 直接字段名 `iosSimulator`）只允许读取 Host 当前台前任务的公开模拟器状态，并请求打开既有
...
- **测试门槛**：命中本节的 PR 必须有"从旧状态升级"的自动化用例——fixture 造出老布局
  ...
  根命令 `pnpm demo:legacy-migration` 提供一条额外的人工可读验收：只在 `os.tmpdir()`
  构造市场插件、用户停用插件和带技能插件，独立 CLI 复用正式
  `ghostSnapshotWorkerProcess` 的请求处理器，并对 receipt、技能快照、一次性门和回滚不改
  安装目录做硬断言；任何一项失败都必须非零退出。该 demo 不替代自动化用例，也不等于
  真实用户目录原地升级已经完成。
...
6. Forge（scaffold／pack／install）是否排除了 Host 受管根（安装根 + 批准状态根），且按
   realpath **双向**判定（源目录既不在受管根内、也不是它的祖先）、挡住大小写与软链／
   junction 别名？递归收集是否不跟随链接进受管根？`ghost_forge_pack` 是否仍然只产出
   `.cindy`、没有隐式安装；`ghost_forge_install` 是否只在用户明确要求时调用并复用本地
   安装／更新事务？
   ...
   状态根路径、`.cindy` 包格式、管子协议、内置 id，或安装状态记录／能力字段？命中就
   逐条问：用户升级后**什么都不做**时，已装、已批准、已启用的插件是否照旧可用？新增的
   必填字段／新校验是否自带从旧版授权事实的 backfill，而不是把"老数据缺字段"当篡改直接
   fail closed？迁移是否只在"从未有过 receipt／已判损坏"时发生、权限集原样不扩权、来源
   有日志？自动做不到时是否有明确提示 + 一次性批量恢复入口（不是"去市场逐个重装"）、
   且不清掉用户已存的凭证与偏好（已装、启停状态、凭证与偏好都要保留）？新状态被旧版本
   读到是否不判损坏（可回滚）？是否有基于旧布局 fixture 的升级用例，而不是只测全新安装？
   让存量插件失效且无迁移 = P0，需把关人对该影响明确确认。
...
10. 复用 Cindy／Meka 插件管理页面时，共享市场操作是否全部经过当前 surface 的渠道适配器？
