...
| `apps/claude-code-bin`、`apps/codex-bin`、`apps/ripgrep-bin`、`apps/codex-package-bin` | agent runtime 二进制（按平台分目录），不进 git 历史。桌面端运行／打包用的是**目录分发**的 `apps/codex-package-bin`（pin `tools/codex-package/latest.json`）；`apps/codex-bin` 是**发布链路专用**的**单文件** codex runtime（pin `tools/codex/latest.json`），不参与 dev/postinstall 自举。CDN 上的应用 manifest **两种形态都要发**：单文件 `codex`（由 `apps/codex-bin` 收集，留给 ≤0.0.20 客户端与 MCPRouter linux runtime manifest）与目录分发 `codexPackage`（**直接复用 pin 直链字节**，≥0.0.21 桌面端启动消费）。见 `docs/dev-rules/agent-runtime-release.md`「发布物 runtime ≠ 桌面端打包 runtime」 |
