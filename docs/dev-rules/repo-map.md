...
| `apps/claude-code-bin`、`apps/codex-bin`、`apps/ripgrep-bin`、`apps/codex-package-bin` | agent runtime 二进制（按平台分目录），不进 git 历史。桌面端运行／打包用的是**目录分发**的 `apps/codex-package-bin`（pin `tools/codex-package/latest.json`）；`apps/codex-bin` 是**发布链路专用**的**单文件** codex runtime（pin `tools/codex/latest.json`，CDN runtime 对象用它），不参与 dev/postinstall 自举。见 `docs/dev-rules/agent-runtime-release.md`「发布物 runtime ≠ 桌面端打包 runtime」 |
