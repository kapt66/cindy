...
- 新建自定义项目时，项目路径既可手动输入，也可通过原生“选择目录”按钮选择；取消选择
  或选择器异常不会改变草稿。项目保存后路径保持只读，原地移动由数据层拒绝（无需在编辑页
  暴露移动操作）。该按钮曾在 `fc7a77b6c`（2026-07-27，项目/角色迁移）重建编辑器时遗漏，
  `440722740`（2026-08-03）仅恢复手动创建路径，本次补回选择器。
