...
import { runGhostSnapshotWorkerRequest } from "../apps/desktop/src/main/cindy-brain/ghostSnapshotWorkerProcess.ts";
...
function assertDemo(condition: unknown, message: string): asserts condition {
  if (!condition)
    throw new Error(`legacy migration demo assertion failed: ${message}`);
}

async function main(): Promise<void> {
  const workDir = await fs.promises.mkdtemp(
    path.join(os.tmpdir(), "cindy-legacy-migration-demo-"),
  );
  ...
  await plantLegacyInstall(
    ...
    manifest("skill-plugin", {
      ...
      skill: {
        items: [
          { dir: "skills/demo", name: "demo", description: "Demo skill" },
        ],
      },
    }),
    {
  ...
  const manager = new GhostManager({
    ...
    // 独立 tsx 进程没有 Electron 打包后的 utility worker 路径；复用 worker 的同一
    // 请求处理器执行快照操作，避免 demo 因运行宿主不同而把技能迁移误报为失败。
    ...
    log: {
      info: () => undefined,
      warn: (message, meta) =>
        console.warn(`[legacy-migration] ${message}`, meta ?? {}),
      error: (message, meta) =>
        console.error(`[legacy-migration] ${message}`, meta ?? {}),
    },
  });

  printList(
    "迁移前(升级刚发生,还没有任何 receipt —— 这就是 #1080 被回滚时用户看到的):",
    manager,
  );
  console.log(
    `${DIM}  ↑ 全部 legacy-unapproved + 停用:开关点不动、要用户逐个重新确认。${RESET}`,
  );
  ...
  assertDemo(
    ...
    `unexpected failed ids: ${outcome.failed.join(", ")}`,
  );
  assertDemo(
    outcome.retryPending.length === 0,
    `unexpected retry ids: ${outcome.retryPending.join(", ")}`,
  );
  assertDemo(
    ["market-plugin", "skill-plugin", "user-disabled"].every((id) =>
      outcome.migrated.includes(id),
    ),
    `not all legacy installs migrated: ${outcome.migrated.join(", ")}`,
  );
  ...
  if (skill) {
    ...
    assertDemo(ok, "skill-plugin approved snapshot did not verify");
  }
  assertDemo(skill, "skill-plugin disappeared after migration");
  ...
  assertDemo(
    second.migrated.length === 0,
    "completed ledger allowed a second migration",
  );
  assertDemo(
    planted?.approval.state === "legacy-unapproved",
    "plugin planted after migration was unexpectedly approved",
  );

  // 回滚余地:迁移只写状态根,安装目录三文件原样保留 —— 回滚到旧客户端仍可用。
  const marketGhostJson = fs.readFileSync(
    path.join(installRoot, "market-plugin", "ghost.json"),
    "utf8",
  );
  console.log(
    `\n${BOLD}回滚余地${RESET}: 安装目录 ghost.json 未被迁移改动 = ${
      marketGhostJson.includes('"market-plugin"')
        ? `${GREEN}true${RESET}`
        : `${YELLOW}false${RESET}`
    } (旧客户端回滚后仍从安装目录判定,不错位)`,
  );
  assertDemo(
    marketGhostJson.includes('"market-plugin"'),
    "legacy manifest changed during migration",
  );
