...
async function main() {
  ...
  const rendererTargets = list.filter(
    (t) => t.type === "page" && t.url.startsWith("http://localhost:5173"),
  );
  const target = rendererTargets.find((t) => {
    const parsed = new URL(t.url);
    return parsed.search === "" && !parsed.hash.startsWith("#/sidebar-window");
  });
  if (!target) {
    console.error("renderer page target not found");
    process.exit(3);
  }
  ...
  if (cmd === "eval") {
    ...
    ws.on("message", (raw) => {
      ...
      if (m.id === 1) {
        if (m.error) {
          console.error("cdp error", m.error);
          process.exit(6);
        }
