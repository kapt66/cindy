...
test("stripNonProse: 剥离 i18next 插值,避免变量名与术语同形误报", () => {
  ...
  assert.equal(
    occursIn(stripNonProse("在 {{project}} 中新建"), "project"),
    false,
  );
...
test("stripNonProse: 剥离 URL、邮箱与文件名", () => {
  assert.equal(
    occursIn(stripNonProse("打开 https://x.com/agent/list"), "agent"),
    false,
  );
  assert.equal(
    occursIn(stripNonProse("读取 project.json 失败"), "project"),
    false,
  );
  assert.equal(
    occursIn(stripNonProse("联系 agent@example.com"), "agent"),
    false,
  );
});
...
test("findCaseMismatch: 扫描全部匹配,不因首个正确就漏掉后面的错误形态", () => {
  ...
  assert.equal(
    findCaseMismatch("创建 Worker 后，该 worker 会自动启动", "Worker"),
    "worker",
  );
...
test("makeExemptChecker: 以点结尾的子树前缀豁免整段", () => {
  ...
  assert.equal(
    isExempt("desktop:settings.remoteControl.hook"),
    false,
    "前缀必须含点,不能误伤 remoteControl",
  );
});
...
const glossary = JSON.parse(
  fs.readFileSync(path.join(ROOT, "i18n", "glossary.json"), "utf8"),
);
...
test("glossary.json: 每条术语都有裁决理由", () => {
  for (const term of glossary.terms) {
    assert.ok(
      term.note?.trim(),
      `术语 ${term.id} 缺 note——没有理由的裁决会被后人反复推翻`,
    );
  }
});
...
test("glossary.json: 声明的译法必须是现状主流，否则要写明为何有意偏离", () => {
    ...
    flatten(
      JSON.parse(
        fs.readFileSync(
          path.join(
            ROOT,
            `apps/desktop/src/renderer/i18n/locales/${locale}/common.json`,
          ),
          "utf8",
        ),
      ),
  ...
  for (const term of glossary.terms) {
    ...
    const re = new RegExp(
      `(?<![A-Za-z0-9_-])${escaped}s?(?![A-Za-z0-9_-])`,
      "i",
    );
    const keys = [...corpus[glossary.sourceLocale]]
      .filter(([, v]) => re.test(v))
      .map(([k]) => k);
    ...
    for (const locale of glossary.locales) {
      ...
      const accepted = [
        declared,
        ...(term.alsoAllowed?.[locale] ?? []).map((v) => v.text),
      ];
...
test("glossary.json: locales 与 desktop SUPPORTED_LOCALES 一致", () => {
  const localeTs = fs.readFileSync(
    path.join(ROOT, "apps/desktop/src/shared/locale.ts"),
    "utf8",
  );
  const match = localeTs.match(/SUPPORTED_LOCALES\s*=\s*\[([^\]]*)\]/);
  assert.ok(match, "无法从 locale.ts 解析 SUPPORTED_LOCALES");
  const supported = [...match[1].matchAll(/['"]([^'"]+)['"]/g)].map(
    (m) => m[1],
  );
  assert.deepEqual(
...
test("glossary-baseline.json: 条目格式合法且无重复", () => {
  const baseline = JSON.parse(
    fs.readFileSync(path.join(ROOT, "i18n", "glossary-baseline.json"), "utf8"),
  );
  ...
  for (const entry of entries) {
    ...
    assert.ok(
      glossary.locales.includes(parts[0]),
      `baseline 条目语言 "${parts[0]}" 不在术语表 locales 内`,
    );
    assert.match(
      parts[4],
      /^[0-9a-f]{12}$/,
      `baseline 条目摘要格式不对: ${parts[4]}`,
    );
  }
});
...
test("validateAgainstSchema: 正本必须符合自己的 schema", () => {
  const schema = JSON.parse(
    fs.readFileSync(path.join(ROOT, "i18n", "glossary.schema.json"), "utf8"),
  );
...
test("validateAgainstSchema: 校验 required / enum / pattern / oneOf", () => {
  ...
  assert.deepEqual(
    validateAgainstSchema({ id: "session", status: "decided" }, schema),
    [],
  );
  assert.match(
    validateAgainstSchema({ id: "session" }, schema).join(),
    /缺少必填字段 "status"/,
  );
  assert.match(
    validateAgainstSchema({ id: "Session", status: "decided" }, schema).join(),
    /不匹配/,
  );
  assert.match(
    validateAgainstSchema({ id: "a", status: "decied" }, schema).join(),
    /取值必须是/,
  );
  // 条件禁用少写 whenEn 时,两个 oneOf 分支都不匹配
  assert.match(
    validateAgainstSchema(
      { id: "a", status: "decided", forbidden: [{ text: "代理" }] },
      schema,
    ).join(),
    /oneOf/,
  );
...
test("normalizeForPunctuation: 插值后的半角标点能被检出", () => {
  ...
  assert.equal(
    findHalfWidthPunct(stripNonProse(raw)),
    null,
    "这正是原先漏检的成因",
  );
...
test("normalizeForPunctuation: 文件名与 URL 里的冒号不算标点违规", () => {
  ...
  assert.equal(
    findHalfWidthPunct(normalizeForPunctuation("请检查 config.json:12 行")),
    null,
  );
  assert.equal(
    findHalfWidthPunct(
      normalizeForPunctuation("详见 https://a.example/b:8080 页面"),
    ),
    null,
  );
});
...
test("stripNonProse: URL 后紧跟中文时,不能把后面的正文一起吞掉", () => {
  ...
  assert.ok(
    stripped.includes("返回对话列表继续操作"),
    `URL 之后的正文被吞了:${JSON.stringify(stripped)}`,
  );
  ...
  for (const term of glossary.terms) {
    for (const field of [
      ...
      for (const locale of Object.keys(term[field] ?? {})) {
        assert.ok(
          locales.has(locale),
          `术语 ${term.id} 的 ${field}.${locale} 语言键不在 locales 里`,
        );
      }
    }
  }
...
test("stripNonProse: 任意扩展名的文件名都要剥离,但不能吃掉数字", () => {
  ...
  assert.equal(
    findHalfWidthPunct(normalizeForPunctuation("已用 1.5 GB，缓存超限,请清理")),
    ",",
  );
});
...
test("findHalfWidthPunct: 右括号等闭合符号也算中文正文的左边界", () => {
  assert.equal(
    findHalfWidthPunct("可润色改写正文(直接替换),或用卡片替换"),
    ",",
  );
  assert.equal(
    findHalfWidthPunct("卸载「{{name}}」?".replace("{{name}}", "x")),
    "?",
  );
...
test("禁用译法的提示不给替换目标,只给英文源", () => {
  ...
  const forbiddenStart = src.search(/rule:\s*["']forbidden-term["']/);
  const termCaseStart = src.search(/rule:\s*["']term-case["']/);
  assert.ok(forbiddenStart >= 0, "未定位到 forbidden-term 规则");
  assert.ok(termCaseStart > forbiddenStart, "未定位到 term-case 规则");
  const forbiddenBlock = src
    .slice(forbiddenStart, termCaseStart)
    // 剔除注释行:注释里会引用旧的「应为」句式来解释为什么不再用它
    .split("\n")
    .filter((line) => !line.trim().startsWith("//"))
    .join("\n");
  assert.ok(
    forbiddenBlock.includes("hint:"),
    "未定位到 forbidden-term 的提示构造",
  );
  assert.ok(
  ...
  assert.ok(
    forbiddenBlock.includes("英文源"),
    "forbidden-term 的提示应附上英文源供判断语境",
  );
});
...
test("GLOSSARY.md 必须声明「参考，不是替换表」", () => {
  ...
  assert.ok(
    doc.includes("禁止拿本表做脚本批量替换"),
    "GLOSSARY.md 缺少批量替换禁令",
  );
});
...
test("validateAgainstSchema: 关键字的值形状写错必须报错而不是被曲解", () => {
  ...
  assert.doesNotThrow(() =>
    validateAgainstSchema({}, { type: "object", additionalProperties: false }),
  );
  assert.doesNotThrow(() =>
    validateAgainstSchema(
      {},
...
test("normalizeForPunctuation: URL / 邮箱后紧跟半角标点 + 中文时留下正文边界", () => {
  ...
  assert.equal(
    findHalfWidthPunct(
      normalizeForPunctuation("请访问 https://x.test,返回对话列表"),
    ),
    ",",
  );
  assert.equal(
    findHalfWidthPunct(normalizeForPunctuation("联系 a@x.com,重启后重试")),
    ",",
  );
  assert.equal(
    findHalfWidthPunct(normalizeForPunctuation("联系 a@x.com;然后重启")),
    ";",
  );
...
test("countOccurrences: 次数进 fingerprint 才能挡住「已冻结 key 里新增一处」", () => {
  ...
  assert.equal(
    countOccurrences("创建 Worker 后 worker 会启动", "Worker", {
...
test("validateAgainstSchema: boolean schema 的 false 必须恒失败", () => {
  ...
  assert.deepEqual(
    validateAgainstSchema(["x"], { type: "array", items: true }),
    [],
  );
...
test("stripNonProse: 邮箱 local part 不能把紧贴在前面的中文正文吃掉", () => {
  ...
  assert.ok(
    stripped.includes("worker"),
    `正文被吞了:${JSON.stringify(stripped)}`,
  );
...
test("glossary.json: punctuationExempt 格式合法且不滥用", () => {
  ...
  assert.ok(
    list.length <= 5,
    `标点豁免已达 ${list.length} 条,请复核规则本身是否需要调整`,
  );
});
...
test("normalizeForPunctuation: 文件名后的正文标点与 URL 后的省略号都要保留", () => {
  ...
  assert.equal(
    findHalfWidthPunct(normalizeForPunctuation("编辑 config.json,然后重试")),
    ",",
  );
  assert.equal(
    findHalfWidthPunct(normalizeForPunctuation("编辑 config.json, 然后重试")),
  ...
  // 路径里的冒号仍不算正文标点
  assert.equal(
    findHalfWidthPunct(normalizeForPunctuation("详见 config.json:12 行")),
    null,
  );
  // token 不能吃掉正文省略号
  assert.equal(
    hasAsciiEllipsis(normalizeForPunctuation("Open https://x.test...")),
    true,
  );
  assert.equal(
    hasAsciiEllipsis(normalizeForPunctuation("联系 a@x.com...然后重试")),
    true,
  );
  // URL 内部的单点不受影响
  assert.equal(
    hasAsciiEllipsis(normalizeForPunctuation("见 https://a.test/a.b.c 页面")),
    false,
  );
});
...
test("findHalfWidthPunct: 连字符 / 下划线结尾的 code token 后也算左边界", () => {
  ...
  // 逗号前是连字符,只认字母数字会整类漏掉
  assert.equal(
    findHalfWidthPunct("保留前缀 cindy-,仅随应用内置的插件可用"),
    ",",
  );
...
test("TOKEN_TAIL: 正文直接贴在 URL / 邮箱后面(连标点都没有)时也要截断", () => {
  ...
  assert.ok(
    stripNonProse("访问 https://x.test返回worker操作").includes("worker"),
  );
  assert.ok(stripNonProse("联系 a@x.com然后worker操作").includes("worker"));
  // 地址本身仍然被完整剥掉,不会把 host 里的词当正文
  assert.ok(
    !stripNonProse("访问 https://worker.test返回操作").includes("worker.test"),
  );
  // 端口号、query string 这类合法 URL 结构不能被切坏(切坏了会把里面的半角标点当正文标点)
  assert.equal(
    findHalfWidthPunct(
      normalizeForPunctuation("见 https://a.test:8080/x?ids=1,2 页面"),
    ),
    null,
  );
});
...
test("caseStandardFor: alsoAllowed 允许英文原词时同样要查大小写", () => {
  ...
  assert.equal(
    caseStandardFor(
      { en: "Agent", translations: { "zh-CN": "Agent" } },
      "zh-CN",
    ),
    "Agent",
  );
  ...
  assert.equal(
    caseStandardFor({ en: "Agent", translations: { "zh-CN": "Agent" } }, "en"),
    null,
  );
});
