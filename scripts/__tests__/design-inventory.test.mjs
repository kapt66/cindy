...
test('extractRouterFacts: 真实 router.tsx 的三类去向逐条钉死', () => {
  ...
  assert.deepEqual(production.map((row) => `${row.path} ${row.component}`), [
    ...
    '/cc-agent/meka MekaProjectRoleEditorRoute',
    '/cc-agent/meka/plugins GhostPluginPage',
    '/cc-agent/meka/skills MekaSkillHomeView',
    '/cc-agent/meka/skills/market MekaSkillMarketListView',
  ...
  assert.deepEqual(redirects.map((row) => `${row.path} -> ${row.to}`), [
    ...
    '/meka-plugins -> /cc-agent/meka/plugins',
