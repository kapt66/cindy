...
test("default isolated userData path is region-aware", () => {
	assert.equal(path.basename(defaultIsolatedUserDataDir("", "global")), "CindyMeka-dev2");
	assert.equal(path.basename(defaultIsolatedUserDataDir("", "cn")), "CindyMeka-dev2");
	assert.equal(path.basename(defaultIsolatedUserDataDir("review", "dev")), "CindyMekaDev-dev2-review");
});
...
test("isOfficialProductionUserDataDir matches every official region profile", () => {
	...
	assert.ok(officialProductionUserDataDirs().some((dir) => path.basename(dir) === "CindyMeka"));
	assert.ok(officialProductionUserDataDirs().some((dir) => path.basename(dir) === "CindyMekaDev"));
});
