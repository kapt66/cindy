...
test("macOS Terminal launch runs command before activating Terminal", () => {
  const lines = appleScriptLines(
    osascriptLaunchDarwinTerminalArgs("echo test"),
  );
...
test("desktop startup timeout has a bounded high-load override", () => {
  assert.equal(resolveStartupReadyTimeoutMs({}), 600_000);
  assert.equal(
    resolveStartupReadyTimeoutMs({ XDT_DESKTOP_STARTUP_TIMEOUT_MS: "1200000" }),
    1_200_000,
  );
  assert.equal(
    resolveStartupReadyTimeoutMs({ XDT_DESKTOP_STARTUP_TIMEOUT_MS: "1" }),
    30_000,
  );
  assert.equal(
    resolveStartupReadyTimeoutMs({ XDT_DESKTOP_STARTUP_TIMEOUT_MS: "9999999" }),
    1_800_000,
  );
  assert.equal(
    resolveStartupReadyTimeoutMs({ XDT_DESKTOP_STARTUP_TIMEOUT_MS: "invalid" }),
    600_000,
  );
});
...
test("desktop restart clears only desktop Vite dev caches", () => {
  ...
  try {
    ...
    const logs = [];
    const removed = clearDesktopDevCaches(repo, {
      logger: { log: (message) => logs.push(message) },
    });

    assert.deepEqual(
      removed.map((entry) => path.relative(repo, entry)),
      [
...
test("restart kill scope only targets the checkout that runs the script", () => {
  ...
  const processes = [
    {
      pid: 1,
      command: `node ${path.join(ownRoot, "node_modules/@electron-forge/cli")} start`,
    },
    {
      pid: 2,
      command: `${path.join(otherRoot, "node_modules/electron/dist/Electron.app/Contents/MacOS/Electron")} .`,
    },
    {
      pid: 3,
      command: `node ${path.join(ownRoot, "node_modules/electron/dist")} --app-path=${path.join(ownRoot, "apps/desktop")}`,
    },
  ];

  const { targets, preserved } = partitionDesktopDevProcesses(
    processes,
    ownRoot,
  );
  assert.deepEqual(
...
test("userData conflict detection matches exact sandbox dirs only", () => {
  ...
  assert.equal(
    commandUsesUserDataDir(
      helper,
      "/Users/dev/Library/Application Support/Cindy-dev-pi-latest",
    ),
    true,
  );
  ...
  assert.equal(
    commandUsesUserDataDir(
      helper,
      "/Users/dev/Library/Application Support/Cindy-dev",
    ),
    false,
  );
  assert.equal(
    commandUsesUserDataDir(
      helper,
      "/Users/dev/Library/Application Support/Cindy",
    ),
    false,
  );
  // 行尾结束与尾斜杠归一。
...
test("shared production userData path is region-aware", () => {
  assert.equal(path.basename(productionUserDataDir()), "CindyMeka");
  assert.equal(path.basename(productionUserDataDir("global")), "CindyMeka");
  assert.equal(path.basename(productionUserDataDir("cn")), "CindyMeka");
  assert.equal(path.basename(productionUserDataDir("dev")), "CindyMekaDev");
});

test("default isolated userData path is region-aware", () => {
  assert.equal(
    path.basename(defaultIsolatedUserDataDir("", "global")),
    "CindyMeka-dev2",
  );
  assert.equal(
    path.basename(defaultIsolatedUserDataDir("", "cn")),
    "CindyMeka-dev2",
  );
  assert.equal(
    path.basename(defaultIsolatedUserDataDir("review", "dev")),
    "CindyMekaDev-dev2-review",
  );
});
...
test("isOfficialProductionUserDataDir matches every official region profile", () => {
  assert.equal(
    isOfficialProductionUserDataDir(productionUserDataDir("cn")),
    true,
  );
  assert.equal(
    isOfficialProductionUserDataDir(productionUserDataDir("global")),
    true,
  );
  assert.equal(
    isOfficialProductionUserDataDir(productionUserDataDir("dev")),
    true,
  );
  assert.equal(
    isOfficialProductionUserDataDir(defaultIsolatedUserDataDir("", "cn")),
    false,
  );
  assert.ok(
    officialProductionUserDataDirs().some(
      (dir) => path.basename(dir) === "CindyMeka",
    ),
  );
  assert.ok(
    officialProductionUserDataDirs().some(
      (dir) => path.basename(dir) === "CindyMekaDev",
    ),
  );
});
...
test("isolated official-profile refuse happens before mkdir in the restart main flow", () => {
  ...
  const refuseIdx = source.indexOf(
...
test("preserve-running only shares a target with live records from the same region", () => {
  const userData = fs.mkdtempSync(
    path.join(os.tmpdir(), "cindy-shared-region-"),
  );
  ...
  try {
    ...
    assert.deepEqual(
      inspectSharedUserDataRegion(userData, "global", processes),
      {
        compatible: true,
        reason: null,
      },
...
test("desktop restart rejects an unmerged migration before the kill step when --shared is explicit", () => {
  ...
  const calls = [];
  try {
    fs.mkdirSync(path.join(repo, "apps", "desktop", "drizzle"), {
      recursive: true,
    });
    fs.writeFileSync(
      path.join(repo, "apps", "desktop", "drizzle", "0000_init.sql"),
      "SELECT 0;\n",
    );
    const git = (...args) => {
    ...
    fs.writeFileSync(
      path.join(repo, "apps", "desktop", "drizzle", "0001_feature.sql"),
      "SELECT 1;\n",
    );

    assert.throws(
      () =>
        runDesktopRestart(["--wait-ready", "--", "--shared"], repo, (step) =>
          calls.push(step),
        ),
...
test("desktop readiness status is parsed only after an atomic status file appears", () => {
  ...
  try {
    ...
    assert.deepEqual(readDesktopStartupStatus(statusPath), {
      state: "ready",
      pid: 123,
    });
  } finally {
...
test("desktop readiness waiter removes an acknowledged ready status", async () => {
  const statusPath = fileURLToPath(
    new URL(`./startup-ready-${process.pid}.json`, import.meta.url),
  );
  try {
    fs.writeFileSync(statusPath, '{"state":"ready","pid":123}\n');
...
test("desktop readiness waiter keeps waiting after ready-to-show and surfaces database failure", async () => {
  const statusPath = fileURLToPath(
    new URL(`./startup-db-failed-${process.pid}.json`, import.meta.url),
  );
  try {
    ...
    setTimeout(() => {
      fs.writeFileSync(
        ...
        JSON.stringify({
          ...
          code: "MIGRATE_FAILED",
          message:
            "applied migration runtime identity changed at seq 77 (0077_nebulous_veda.sql)",
        }) + "\n",
      );
    }, 10);
...
test("desktop readiness timeout leaves an abandoned tombstone for late Electron events", async () => {
  const statusPath = fileURLToPath(
    new URL(`./startup-timeout-${process.pid}.json`, import.meta.url),
  );
  try {
    fs.writeFileSync(statusPath, '{"state":"pending"}\n');
    await assert.rejects(
      waitForDesktopStartup(statusPath, 0),
      /did not finish window\/auth\/database startup/,
    );
...
test("desktop whoami identifies multiple passive previews sharing one userData", () => {
  ...
  const electronHelper = path.join(
    previewRoot,
    "node_modules",
    "electron",
    "helper",
  ...
  const electronMainTwo = path.join(
    previewRootTwo,
    "node_modules",
    "electron",
    "dist",
  ...
  const electronHelperTwo = path.join(
    previewRootTwo,
    "node_modules",
    "electron",
    "helper",
  ...
  const processes = [
    { pid: 10, ppid: 9, command: `${electronMain} .` },
    {
      pid: 11,
      ppid: 10,
      command: `${electronHelper} --type=renderer --user-data-dir=${userData} --app-path=${appPath}`,
    },
    {
      pid: 9,
      ppid: 8,
      command: `XDT_SCHEDULER_PASSIVE='1' node ${devEnv} electron-forge start`,
    },
    { pid: 20, ppid: 19, command: `${electronMainTwo} .` },
    {
      pid: 21,
      ppid: 20,
      command: `${electronHelperTwo} --type=renderer --user-data-dir=${userData} --app-path=${appPathTwo}`,
    },
    {
      pid: 19,
      ppid: 18,
      command: `set "XDT_SCHEDULER_PASSIVE=1" && node ${devEnvTwo} electron-forge start`,
    },
  ];
...
test("desktop whoami ignores the unquoted crashpad userData argument", () => {
  const previewRoot = path.resolve("/repo/cindy-preview");
  const userData = path.resolve("/tmp/Cindy-dev2-preview");
  const worktrees = [{ rootDir: previewRoot, branch: "carol/preview/example" }];
  const electronMain = path.join(
    previewRoot,
    "node_modules",
    "electron",
  ...
  const processes = [
    { pid: 10, ppid: 9, command: `${electronMain} .` },
    {
      pid: 11,
      ppid: 10,
      command: `${electronMain} --type=crashpad-handler --user-data-dir=${userData} /prefetch:4`,
    },
    {
      pid: 12,
      ppid: 10,
      command: `${electronMain} --type=renderer --user-data-dir="${userData}" --app-path="${appPath}"`,
    },
  ];
  ...
  assert.equal(instances[0].userDataDir, userData);
});
...
test("passive previews do not use a one-slot userData lock", () => {
  ...
  assert.equal(
    fs.existsSync(
      new URL("../../apps/desktop/src/main/passiveDevLock.ts", import.meta.url),
    ),
    false,
  );
});
...
test("desktop whoami prefers launch-time commit metadata over process inference", () => {
  const worktrees = [
    { rootDir: "/repo/cindy-preview", branch: "carol/preview/example" },
  ];
  const scanned = [
    {
      pid: 10,
  ...
  const merged = mergeDesktopInstanceRecords(
    scanned,
    [
      {
        pid: 10,
...
test("devEnvPrefix escapes single quotes in scenario values on POSIX shells", () => {
  const prefix = devEnvPrefix(
    { XDT_LOGIN_SCENARIO: "providers:both'x" },
    "darwin",
  );
...
test("formatDesktopDevVerdict flattens failed messages and keeps next", () => {
  ...
  assert.match(
    text,
    /message=Shared Cindy userData cannot run migration artifacts/,
  );
  assert.match(
    text,
    /next=pnpm restart:desktop:remote -- --isolated=@worktree/,
  );
});

test("isolationNameFromWorktree strips cindy-, adds a path digest, and stays within 32 chars", () => {
  const named = isolationNameFromWorktree(
    "/Users/dash/Code/Cindy/cindy-local-ollama-models",
  );
  ...
  assert.ok(
    isolationNameFromWorktree(`/tmp/cindy-${"a".repeat(80)}`).length <= 32,
  );
});
...
test("resolveIsolatedArg expands @worktree to a named sandbox", () => {
  ...
  assert.equal(
    resolveIsolatedArg("--isolated=feature-a", "/repo/x"),
    "--isolated=feature-a",
  );
...
test("shouldRefuseHostedRestart blocks same-checkout hosts and other-checkout shared starts", () => {
  ...
  assert.equal(
    shouldRefuseHostedRestart(null, {
...
test("isolated failures do not suggest another isolated start", () => {
  assert.equal(
    shouldSuggestIsolatedNext({ isolated: true, code: "MIGRATE_FAILED" }),
    false,
  );
  const verdict = buildDesktopDevVerdictFromFailure(
    Object.assign(new Error("[MIGRATE_FAILED] applied identity changed"), {
      startupStatus: {
        code: "MIGRATE_FAILED",
        message: "applied identity changed",
      },
    }),
...
test("inferDesktopDevFailureCode reads tagged startup failures", () => {
  assert.equal(
    inferDesktopDevFailureCode("[SINGLE_INSTANCE_OWNED] owned"),
    "SINGLE_INSTANCE_OWNED",
  );
  assert.equal(
    inferDesktopDevFailureCode(
      "Desktop dev did not finish window/auth/database startup within 120s.",
    ),
    "STARTUP_TIMEOUT",
  );
});
...
test("host identity uses the same conservative path match as kill", () => {
  const own = "/repo/cindy-feature";
  assert.equal(
    commandContainsPath(`electron-forge start ${own} --isolated=feature`, own),
    false,
  );
...
test("restartContextFromArgv reads --region cn and XDT_ISOLATED", () => {
  ...
  assert.equal(
    restartContextFromArgv([], { XDT_ISOLATED: "1" }).isolated,
    true,
  );
  assert.equal(
...
test("named isolated does not keep another checkout's inherited userData dir", () => {
  const derived = defaultIsolatedUserDataDir(
    "local-ollama-models-abc123",
    "global",
  );
  assert.equal(
    inheritedUserDataBlocksNamedIsolation(
...
test("assertDesktopRestartStepSucceeded throws so runner can print a verdict", () => {
  assert.throws(
    ...
    (error) =>
      error instanceof DesktopRestartStepError &&
      ...
      assertDesktopRestartStepSucceeded(
        {
          ...
          args: ["restart.mjs", "--wait-ready"],
        },
        { status: 2 },
      ),
    (error) =>
      error instanceof DesktopRestartStepError &&
      error.alreadyHasVerdict === true,
  );
  assert.throws(
    () =>
      assertDesktopRestartStepSucceeded(
