...
只有当 Host 注入的项目参考资料（`moduleEditorSkillPath`、`damageEncodingRulePath`）未覆盖当前运行时
语义、资料之间冲突，或与客户端证据冲突时，才创建或派发一次只读 MCPR Worker；资料已覆盖且无冲突时，
引用资料路径与结论即可，不得为了形式再派发。派发时任务正文必须同时包含
...
single-skill 任务只允许用户绑定的目标技能 ID；table-scope 任务只允许用户已确认的 `targetSkillIds`
集合内的目标；两者都不得导出、读取或向 Worker 传递参考技能 ID 或 reference marker。
...
确需派发时，环境 ready 且需求已足以描述待核查的服务器语义后，应先完成模块优先证据包，再创建服务器
核查 Worker；不得在没有目标技能导出回执、协议字段和原子能力矩阵时派发。目标资产不存在的结构化
回执本身就是新建场景的目标证据，不得为补“模块图”读取其它技能。`table-scope` **不逐目标派发**：服务器
路由键只为唯一正整数绑定目标注入，表范围没有该绑定，`create_worker` 的 `remote_host_id` 相等校验和
报告校验都必然不匹配；资料未覆盖又必须核查时，改绑范围内一个用户已确认的技能 ID，退回单技能流程完成
这次核查（一次只绑一个 ID，报告的 `targetSkillId` 写该 ID）。不把未经用户确认的目标纳入核查范围。
Worker 核对矩阵中所有依赖运行时解释的原子语义；已由模块组合表达的部分不重新发明实现，但仍需确认当前
远端消费者、目标/时序/数据函数编码兼容。不能把“没有完整专用函数”推导成整组技能不支持。客户端探索
必须从已知配置、导出或消费者路径开始，`rg` 要限定文本文件或具体目录，不递归读取 Unity 根目录、
Library、Temp、Logs、二进制资源或锁文件。
...
与 Lead 绑定值一致的正整数 `targetSkillId`（table-scope 改绑单技能后写该绑定 ID）、`supportStatus`、
`readOnlyConfirmed`、`repository`、
...
本地 Lead 的只读 Shell 查询只允许这三种形态：单一 `rg`（显式给出文件或具体目录，不使用 `--files`）、
单一 `Get-Content -LiteralPath`/`-Path`（注入 ReadCommand 自带的 `-Encoding UTF8` 逐字复制，不得删改），
或单一 `Select-String -Path '<一个文件>' -Pattern '<一个模式>'`；不使用 `rg --files`、`Get-ChildItem`、
`dir`、`find` 等枚举形态（`saga2_unity` / `saga2_json` 下的枚举一律被拒绝）。服务器 Worker 只能使用
...
不能凭外观解释为 p% 攻击力。Lead 已在任务正文引用项目规则（`damageEncodingRulePath`）中的伤害编码
时，Worker 核实它与当前远端 HEAD 是否一致：一致即返回 `supported`，冲突则返回 `uncertain` 或
`unsupported` 并指出冲突点，不得把「项目规则已覆盖、未要求服务器复核」当成阻断写入的理由。
若 Lead 的计划把 `[1,p]` 当百分比，Worker 应直接返回 `unsupported` 并指出编码错误，禁止替 Lead 静默
改写配置。

服务器 Worker 的只读查询不设次数上限；证据不足时可以继续逐条查询，但不要通过重复请求、
换用同类命令或新建 Worker 来延长探索，取得充分证据后立即输出唯一的 `serverCapabilityReport`
JSON；证据不足时使用 `uncertain`。

Worker 的默认取证顺序固定为：先 `git show HEAD:AGENTS.md` 读取规则，再用
`git show -s --format=%H HEAD` 固定 HEAD，然后用
`git grep -l -E <精确符号表达式> HEAD -- internal/battle` 取得当前 HEAD 中的真实路径；随后对
这些真实路径分别使用
...
只有直接符号完全无命中时，才把后续调用用于行为注册表或相邻枚举定义。
