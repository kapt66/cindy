...
方案是**内部**检查点：**不要**把 `[SAGA2_COMBAT_CONFIG_PLAN]` 区块抄进对话，除非用户明确要求看它；
方案**从来不是**用户需要批准的关口——用户已经要求改动且范围已明确时直接执行，不要停下来等批准
（`table-scope` 的写入前确认是范围确认，不是方案审批）；服务器代码只作能力证据，不列入 `surfaces`。
