...
fn run_inner<F: FnMut(InstallerEvent)>(
    ...
    // A malformed or wrong-product ZIP is terminal: refuse before backup or
    // replacement, and discard the archive so Retry cannot reuse it.
    validate_extracted_main_executable(&extract_dir, &args.exe_name)
        .map_err(|error| InstallerFailure::new(error.to_string(), false))?;
...
mod tests {
    ...
    use super::{
        ...
        uses_elevated_private_staging, uses_private_staging_acl, uses_protected_staging,
        validate_extracted_main_executable, Phase,
    };
    use crate::args::{CliArgs, ThemeArg};
    use clap::Parser;
    use sha2::Digest;
