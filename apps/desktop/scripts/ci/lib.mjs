 ...
 * 未就绪,发布目标另议,不随本次翻转。
 */
export const PACKAGED_APP_NAME = 'CindyMeka';
...
/** Cindy Meka 不能把产物误发到上游 Cindy 或旧 XDMaker Meka 前缀。 */
 ...
 * @param {{ signIdentity: string, teamId: string, timestamp?: boolean }} identity
 */
...
export function createMacDMG(appPath, dmgPath, volumeName, identity) {
  ...
  const appName = path.basename(appPath); // CindyMeka.app / CindyMekaDev.app
...
export function runSmokeTest(platform, arch, region = 'global') {
  ...
  const result = spawnSync(
    ...
    [
      ...
      `--platform=${platform}`,
      `--arch=${arch}`,
      // 产物基名按区域派生(cn/global 'CindyMeka' / dev 'CindyMekaDev')。
