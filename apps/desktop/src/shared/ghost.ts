...
export interface GhostPanelDecl {
  ...
  /** Allow the user to override this docked panel's Meka presentation mode. */
...
export interface GhostManifest {
  ...
  /** 随包本地 Node 工作进程声明。 */
  node?: GhostNodeNeeds;
  /** MCPRouter 逻辑 route 白名单详单；声明即获得 mcpr 能力。 */
  mcpr?: GhostMcprNeeds;
  /**
  ...
  workspace?: true;
  iosSimulator?: true;
  /** 在系统文件管理器中定位本机文件或文件夹（不读内容、不回传路径）。 */
  reveal?: true;
...
export function ghostContentKeys(manifest: GhostManifest): string[] {
  ...
  if (manifest.mcpr) keys.push('slotMcpr');
  if (manifest.reveal === true) keys.push('slotReveal');
  return keys;
}
...
function dropEmptyLegacyCapabilitySlots(
  ...
  return slots.filter((slot) => {
    ...
    if (normalized === 'mcpr') return value.mcpr !== undefined;
    if (normalized === 'reveal') return value.reveal === true;
    return true;
  });
}
  ...
  return {
    ...
    manifest: {
      ...
      ...(card !== undefined || prepared.v3BaseCard || slots.includes('card')
        ...
        : {}),
      ...(mcpr !== undefined ? { mcpr } : {}),
      ...(slots.includes('reveal') ? { reveal: true as const } : {}),
