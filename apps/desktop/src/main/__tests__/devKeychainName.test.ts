...
const DEV: KeychainMarkerRead = { kind: 'present', value: 'CindyMekaDev\n' };
...
describe('resolveDevKeychainDecision', () => {
  it('有标记 = CindyMekaDev → 跨重启粘住,不看目录内容', () => {
    expect(
      resolveDevKeychainDecision({
        ...base,
        io: io({ readMarker: () => DEV, profileHasData: () => true }),
      }),
    ).toEqual({ kind: 'rename', appName: 'CindyMekaDev' });
  });
  ...
  it('Windows 编辑器手工修复(\r\n 结尾)按恰好一个换行剥离,照常接受', () => {
    expect(
      resolveDevKeychainDecision({
        ...base,
        io: io({ readMarker: () => ({ kind: 'present', value: 'CindyMekaDev\r\n' }) }),
      }),
    ).toEqual({ kind: 'rename', appName: 'CindyMekaDev' });
  });
  ...
  it('全新沙箱 → 原子认领成功后改名', () => {
    ...
    expect(resolveDevKeychainDecision({ ...base, io: io({ claimMarker: claim }) })).toEqual({
      kind: 'rename',
      appName: 'CindyMekaDev',
    });
    expect(claim).toHaveBeenCalledWith('CindyMekaDev');
  });
  ...
  it('无标记且有数据,复查确证 absent = 真旧沙箱 → 永久默认名', () => {
    expect(resolveDevKeychainDecision({ ...base, io: io({ profileHasData: () => true }) })).toEqual(
      { kind: 'keep-default' },
    );
  });
...
describe('覆写目录的非认领启动 = 观察模式(review 反馈 P1 第十四/十五轮)', () => {
  ...
  it('空 profile → 原子认领默认身份,不与并发隔离启动分叉(review 反馈 P1 第十五轮)', () => {
    ...
    const claim = vi.fn<KeychainIdentityIo['claimMarker']>(() => 'claimed');
    expect(resolveDevKeychainDecision({ ...observe, io: io({ claimMarker: claim }) })).toEqual({
      kind: 'keep-default',
    });
    expect(claim).toHaveBeenCalledWith('CindyMeka');
  });
