...
describe('0104 repair cjk fts missing rows', () => {
  ...
  it('messages 表不存在时不抛（全新库链路上由 0103 负责）', () => {
    const db = new Database(':memory:');
    expect(() => migration.run(db)).not.toThrow();
    db.close();
  });
