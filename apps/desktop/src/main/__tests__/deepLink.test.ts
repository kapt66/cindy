...
describe('redactConsumedDeepLinkInArgv', () => {
  it('drops both schemes before restart without selecting or retaining an import', () => {
    ...
    expect(argv).toEqual(['electron', '--flag', 'cindy://session/keep', `${DEEP_LINK_PROTOCOL}://consumed`, `${DEEP_LINK_PROTOCOL}://consumed`]);
  });
  it('removes every import, including invalid and duplicate arguments, while retaining the selected URL for parsing', () => {
    ...
    expect(argv).toEqual(['electron.exe', '--flag', 'cindy://session/keep', `${DEEP_LINK_PROTOCOL}://consumed`, `${DEEP_LINK_PROTOCOL}://consumed`, `${DEEP_LINK_PROTOCOL}://consumed`]);
  ...
  it('replaces a consumed URL without changing unrelated argv entries', () => {
    ...
    expect(argv).toEqual(['electron.exe', '--flag', `${DEEP_LINK_PROTOCOL}://consumed`]);
  });
