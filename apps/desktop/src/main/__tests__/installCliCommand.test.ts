...
describe('CLI_COMMAND_NAME 跟随 edition 品牌', () => {
  it('测试环境(未注入区域 → 默认 global)命令名为 cindymeka', () => {
    expect(CLI_COMMAND_NAME).toBe('cindymeka');
    expect(CLI_LINK_PATH).toBe('/usr/local/bin/cindymeka');
  });

  it('由区域可执行名小写化:global / cn → cindymeka,内部 dev → cindymekadev', () => {
    expect(brandExecutableName('global').toLowerCase()).toBe('cindymeka');
    expect(brandExecutableName('cn').toLowerCase()).toBe('cindymeka');
    expect(brandExecutableName('dev').toLowerCase()).toBe('cindymekadev');
  });
});
...
describe('buildInstallShellCommand', () => {
  it('mkdir -p 链接目录、拒绝真实目录后 ln -sfn 目标到 source', () => {
    ...
    expect(cmd).toBe(
      `mkdir -p '/usr/local/bin' && if [ -d '/usr/local/bin/cindymeka' ] && [ ! -L '/usr/local/bin/cindymeka' ]; then echo '/usr/local/bin/cindymeka is a directory' >&2; exit 1; fi && ln -sfn '/Applications/Cindy.app/Contents/Resources/cli/cindy' '/usr/local/bin/cindymeka'`,
    );
  });

  it('用 -sfn 而非 -sf(macOS 上替换指向目录的旧 symlink,不解引用进目录)', () => {
    const cmd = buildInstallShellCommand(
      '/App/Cindy.app/Contents/Resources/cli/cindy',
      CLI_LINK_PATH,
    );
  ...
  it('含单引号的路径被安全转义', () => {
    const cmd = buildInstallShellCommand(
      `/Users/o'brien/Cindy.app/Contents/Resources/cli/cindy`,
      CLI_LINK_PATH,
    );
...
describe('buildUninstallShellCommand', () => {
  it('仅当目标是 symlink 时才 rm -f(不误删同路径普通文件,并发移除也幂等)', () => {
    expect(buildUninstallShellCommand(CLI_LINK_PATH)).toBe(
      `if [ -L '/usr/local/bin/cindymeka' ]; then rm -f '/usr/local/bin/cindymeka'; fi`,
    );
  });
...
describe('shellSingleQuote', () => {
  ...
  it('普通路径直接包单引号', () => {
    expect(shellSingleQuote('/usr/local/bin/cindymeka')).toBe(`'/usr/local/bin/cindymeka'`);
  });
  ...
  it('识别 osascript 取消授权(-128 / User canceled)', () => {
    ...
    expect(
      isUserCancelledAdmin(
        Object.assign(new Error('failed'), { stderr: 'execution error: ... (-128)' }),
      ),
    ).toBe(true);
  });
