...
describe('reapClaudeOrphansSync', () => {
  ...
  it('kills current main-process children and historical same-region orphans', async () => {
    ...
    const execFileSync = vi.fn((file: string) => {
      ...
      return [
        ...
        `${historicalPid}|99999|C:\\Users\\me\\AppData\\Roaming\\CindyMeka\\claude-code\\claude.exe`,
        `${livePeerPid}|88888|C:\\Users\\me\\AppData\\Roaming\\CindyMeka\\claude-code\\claude.exe`,
  ...
  it('treats PPID <= 4 as dead and PPID === process.pid as alive without probing', async () => {
    ...
    const execFileSync = vi.fn((file: string) => {
      if (file === 'taskkill') return '';
      return [
        `${systemOrphanPid}|4|C:\\Users\\me\\AppData\\Roaming\\CindyMeka\\claude-code\\claude.exe`,
        `${selfChildPid}|${process.pid}|C:\\Users\\me\\AppData\\Roaming\\CindyMeka\\claude-code\\claude.exe`,
      ].join('\n');
    });
  ...
  it('matches all current Cindy Meka Claude binary path marker shapes', async () => {
    const pids = [701, 702, 703];
    const execFileSync = vi.fn((file: string) => {
      if (file === 'taskkill') return '';
      return [
        `${pids[0]}|4|C:\\Users\\me\\AppData\\Roaming\\CindyMeka\\claude-code\\claude.exe`,
        `${pids[1]}|4|C:/Users/me/AppData/Roaming/CindyMeka/claude-code/claude.exe`,
        `${pids[2]}|4|/Users/me/Library/Application Support/CindyMeka/claude-code/claude`,
      ].join('\n');
    });
    const { reapClaudeOrphansSync } = await importReaper({ platform: 'win32', execFileSync });
  ...
  it('claims historical xdt-maker processes as Cindy Meka migration lineage', async () => {
    const historicalPid = 704;
    const execFileSync = vi.fn((file: string) => {
      if (file === 'taskkill') return '';
      return `${historicalPid}|4|C:\\Users\\me\\AppData\\Roaming\\xdt-maker\\claude-code\\claude.exe`;
    });
    const { reapClaudeOrphansSync } = await importReaper({ platform: 'win32', execFileSync });
  ...
  it('matches Windows path markers case-insensitively', async () => {
    ...
    const execFileSync = vi.fn((file: string) => {
      if (file === 'taskkill') return '';
      return `${lowercasePid}|4|C:\\users\\me\\appdata\\roaming\\cindymeka\\claude-code\\claude.exe`;
    });
    const { reapClaudeOrphansSync } = await importReaper({ platform: 'win32', execFileSync });
