...
describe('netlog 产物事后核对(verifyEndpointNetLogCapture)', () => {
  ...
  it('目录项换成了别的 inode → 不通过', () => {
    ...
    const capture = prepareEndpointNetLogFile(logDir)!;
    if (!capture.dirIno) return; // Windows 文件系统可能不提供 inode；实现会保留类型检查并跳过身份比对。
    fs.writeFileSync(capture.file, '{}', 'utf8');
    // Windows 的 64-bit inode 可能已超出 Number.MAX_SAFE_INTEGER，`ino + 1` 会因精度
    // 丢失仍等于原值；从 1/2 中选一个不同值，才能稳定表达“目录身份不匹配”。
    const differentDirIno = capture.dirIno === 1 ? 2 : 1;
