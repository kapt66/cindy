...
describe('maker:event hot path ordering', () => {
  ...
  it('marks Agent Island stopped before provider abort tails can arrive', () => {
    ...
    expectOrder(directAbortSource, 'if (!sess) return;', 'handleAgentIslandSessionStopped(sess);');
  ...
  it('keeps Codex subscription value out of real session cost totals', () => {
    ...
    expect(codexDoneSource).toMatch(
      /const modelPromise =\s*turnModelPromiseBySession\.get\(session\.id\) \?\? readSessionModelForUsage\(session\.id\);/,
    );
    ...
    expect(codexDoneSource).toMatch(/const isSubscriptionValue =\s*isRemoteCodexSession \|\|/);
    ...
    expect(codexDoneSource).toMatch(
      /const pricing =\s*isSubscriptionValue && !isCodexXaiProviderRoute/,
    );
    ...
    expect(codexDoneSource).toMatch(
      /await recordModelTurnUsage\(\{\s*agentKind: 'codex',\s*model: modelUsageKey,\s*inputTokensDelta: promptTokens,\s*outputTokensDelta: completionTokens,\s*cacheReadTokensDelta: cachedTokens,\s*cacheCreateTokensDelta: 0,\s*\}\)\.finally\(\(\) => rebroadcastCodexTodayUsage\(\)\);[\s\S]*?const pricing =\s*isSubscriptionValue && !isCodexXaiProviderRoute/,
    );
    expect(codexDoneSource).toMatch(
    ...
    expect(modelCostRecordIndex).toBeGreaterThan(
      codexDoneSource.search(/const pricing =\s*isSubscriptionValue && !isCodexXaiProviderRoute/),
    );
    ...
    expect(codexDoneSource).toMatch(/if \(\s*!isRemoteCodexSession &&/);
    expect(codexDoneSource).toContain('!model.startsWith(XAI_MODEL_PREFIX) &&');
    expect(codexDoneSource).toMatch(
      /\(codexAuthInjection === 'env-key' \|\|\s*model\.startsWith\('codex\/'\) \|\|\s*\(sessionProvider === 'xd' && hasGatewayKey\)\)/,
    );
  ...
  it('claude-code 费用走 HYBRID 定价 (gateway 重算 + total_cost_usd 窄兜底) after EVENT broadcast', () => {
    ...
    expect(claudeDoneSource).toMatch(
      /model: isClaudeSubscriptionValueRow\s*\?\s*claudeSubscriptionUsageModelKey\(m\.model\)\s*:\s*m\.model,/,
    );
    expect(claudeDoneSource).toContain(
