...
describe('sendToSession ordering', () => {
  ...
  it('tracks queued Orca accepted callbacks until dispatch settles or the item is discarded', () => {
    ...
    expect(dispatchBlock).toMatch(
      /registerQueuedOrcaInterAgentAcceptedCallback\(\s*clientId,\s*params\.onAccepted,\s*params\.onAcceptedRollback,?\s*\);/,
    );
  ...
  it('awaits Git baseline capture after send_to_session user row persistence and before dispatch', () => {
    ...
    const finalize = changeSetBlock.indexOf(
      "await finalizeTurnChangeSet(session.id, null, 'partial');",
    );
    const secondSeal = changeSetBlock.indexOf(
  ...
  it('keeps worker idle/archive adapters passing the caller lead session id', () => {
    ...
    expect(workerAdapterBlock).toContain(
  ...
  it('exposes Orca lead/worker dispatch receipt fields from the shared DB snapshot', () => {
    ...
    expect(liveBlock).toMatch(
      /return\s+\{\s*ok: true,\s*mode: 'dispatched',\s*clientId,\s*dispatchOutcome: result\.dispatchOutcome,\s*\.\.\.dispatchReceipt,?\s*\};/,
    );
