...
describe('passive shared-userData instance auth isolation', () => {
  ...
  it('稳定登录的 passive initialize 不修复或消费本机 owner 预留', () => {
    ...
    // Meka 分歧(有意保留):本地迁移入口收的是 user 对象而非上游的 userId 字符串——
    // Meka 侧额外用 email 做旧 xdmaker-meka 目录的身份锚点匹配(见
    // legacyUserDataMigration.ts 的 currentUserEmail 与 bootstrap 挂载点注释)。
    // 排序契约不变:迁移必须发生在 passive 闸门之后。
    expect(beforeEnsureReady.indexOf('runLegacyUserDataMigrationForUser(user)')).toBeGreaterThan(
      dbPassiveGuard,
    );
