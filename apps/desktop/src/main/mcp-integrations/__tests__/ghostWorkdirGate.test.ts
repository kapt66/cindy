...
// Meka unity 流程需要断言 UNITY_EDITOR_START_CONFIRM_REQUIRED 分支，因此保留
// 比上游更宽的结果联合类型；成功形态与上游一致。
...
describe('market install live authority', () => {
  function marketHarness(agentKind: TestAgentKind = 'claude-code') {
    ...
    // 第 5 位是 Meka 的 workingDir；上游的 pluginMarket 顺延到第 6 位。
    const deps = makeDeps(agentKind, 's1', 's1-instance', {}, WORKDIR, market as unknown as CindyGhostsHostDeps['pluginMarket']);
    return { deps, market };
  }
