...
import { supportsCindyVersion } from '../../shared/cindyVersion.js';
...
function reportInstallProgress(
  observer: PluginMarketInstallOptions['onProgress'],
  progress: Parameters<NonNullable<PluginMarketInstallOptions['onProgress']>>[0],
): void {
  try {
    observer?.(progress);
  } catch {
...
export interface PluginMarketServiceOptions {
  /** Independent channels must not claim pre-ledger Cindy installations. */
  adoptLegacyInstallations?: boolean;
  /** Independent channels do not inherit Cindy's default install/upgrade policy. */
  applyDefaultInstalls?: boolean;
  /** Optional channel-specific ceiling for already validated package manifests. */
...
export class PluginMarketService {
  ...
  private customIconProjectionGeneration = crypto
    ...
    .toString('hex');
  private readonly sourceStore: MarketSourceStore;
  ...
  constructor(
    ...
    sourceStoreOrOptions:
      | MarketSourceStore
      | PluginMarketServiceOptions = new MarketSourceStore(() =>
      ownerScopedUserDataPath('plugin-market', 'sources.v1.json'),
    ),
    options: PluginMarketServiceOptions = {},
  ) {
    if (sourceStoreOrOptions instanceof MarketSourceStore) {
      this.sourceStore = sourceStoreOrOptions;
      this.options = options;
    } else {
      this.sourceStore = new MarketSourceStore(() =>
        ownerScopedUserDataPath('plugin-market', 'sources.v1.json'),
      );
      this.options = sourceStoreOrOptions;
    }
  }
  ...
  async snapshot(options: PluginMarketSnapshotOptions = {}): Promise<PluginMarketSnapshot> {
    ...
    const serverConfigured = await this.isConfigured();
    ...
    const reconcileDefaults = async (): Promise<'completed' | 'failed'> => {
      if (this.options.applyDefaultInstalls === false) {
        options.onDefaultReconciliationOutcome?.('completed');
        return 'completed';
      }
  ...
  /** Return runtime-present packages whose provenance belongs to this service ledger. */
  ...
  private async installDetail(
    ...
    options: {
      ...
      /** Main-only best-effort progress observer. */
      onProgress?: PluginMarketInstallOptions['onProgress'];
    ...
    try {
      const maxDownloadBytes = this.options.resolveMaxDownloadBytes?.(compatible.manifest);
      ...
      await downloadVerifiedPlugin(download.url, download, tempPath, downloadOptions);
      requireSameMarketOwner(owner);
      try {
  ...
  private async isConfigured(): Promise<boolean> {
    const api = this.api as PluginMarketApi & { isConfigured?: () => Promise<boolean> };
    return typeof api.isConfigured === 'function'
      ? api.isConfigured()
      : Boolean(getClientEndpoint('pluginApiBaseUrl'));
  }
          ...
          await withGhostInstallLock(record.ghostId, async () => {
            ...
            if (approvalEvidence.packageSha256 === null) {
              ...
              if (
                ...
                installedManifestDigest !== ghostManifestDigest(approvalEvidence.approvedManifest)
              ) {
                log.info('disconnected market recovery skipped', {
                  pluginId: record.pluginId,
                  ghostId: record.ghostId,
