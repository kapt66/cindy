...
describe('Plugin Market IPC error boundary', () => {
  ...
  it('preserves structured errors and normalizes unexpected failures', () => {
    ...
    // The merged client also registers the Meka market channel. Every
    // registration must stay behind the same structured error boundary;
    // the union currently has eighteen call sites.
    expect(registerSource.match(/return invokePluginMarket\(/g)?.length).toBe(18);
  });
  ...
  it('routes Meka market handlers through the shared boundary and progress channel', () => {
    ...
    // Meka keeps its own package channel but reuses the same structured error
    // boundary, and streams download progress back to the requesting window.
    expect(body).toContain('return invokePluginMarket(() =>');
    expect(body).toContain('sender.send(MEKA_PLUGIN_MARKET_INSTALL_PROGRESS_CHANNEL, payload);');
    expect(body).toContain('requireInstallOperationId(obj?.operationId)');
