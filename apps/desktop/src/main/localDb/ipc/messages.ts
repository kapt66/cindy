...
/** Compatibility projection used by session-removal cleanup. */
export async function listDeletableSessionPersistedChatAttachmentPaths(
  sessionId: string,
): Promise<string[]> {
  // The persisted-path query is owner-scoped by the active database; the
  // session id is retained in the API for callers that already have it.
  void sessionId;
  return listPersistedChatAttachmentPaths();
}
