...
const h = vi.hoisted(() => ({
  ...
  requestRecycle: vi.fn(
    async (_sessionId: string, _resources?: readonly string[]) => undefined,
  ),
...
beforeEach(() => {
  ...
  // 上游把 status 终态写入改成 withStatusWriteLock → requestWorktreeRecycle;
  // 回收实现未 wire 时该路径直接抛错。本文件只验证广播/锁顺序,按上游自家
  // setSessionsStatusInDb.test.ts 的做法注入 no-op 回收实现。
...
describe('local-db:sessions:update handler wiring', () => {
  ...
  // WL-3.6(`'meka'` 跨层身份契约):`workspaceKind === 'meka'` 必须**同时**带
  // meka_project_id + meka_role_id(反之亦然)。`sessionPatchToRow` 不写这两个身份
  // 字段,所以 update 把普通会话改成 'meka' 只会产出脏身份 ⇒ 必须拒绝,**创建**才是
  // 唯一合法写入 `'meka'` 的路径(create 侧校验 project+role 并登记 recent_workdirs)。
  it('refuses to assign the Meka workspace kind through the generic writer', async () => {
    await expect(invokeUpdate('cc-local', { workspaceKind: 'meka' })).rejects.toThrow(
      /Meka workspace identity can only be assigned/,
    );

    // 拒绝发生在写库之前:行仍是普通对话,身份字段为空。
    expect(
      h.sqlite!
        .prepare(
          'SELECT workspace_kind AS workspaceKind, meka_project_id AS mekaProjectId, meka_role_id AS mekaRoleId FROM sessions WHERE id = ?',
        )
        .get('cc-local'),
    ).toEqual({ workspaceKind: 'dialogue', mekaProjectId: null, mekaRoleId: null });
    // 越权改动不得顺带把工作目录写进最近目录,也不得广播成已生效的 patch。
    expect(h.upsertRecentWorkdir).not.toHaveBeenCalled();
    expect(h.tapWindowBroadcast).not.toHaveBeenCalled();
