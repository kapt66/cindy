...
import { ensureMekaWorkspaceDir, resolveMekaProjectWorkspacePath } from '../mekaWorkspace';
  ...
  ipcMain.handle('local-db:sessions:create', async (event, body) => {
    ...
    if (workspaceKind === 'meka' && hasMekaProjectRole) {
      ...
      if (createBody?.isFormal === true) {
        ...
        if (!formal || !normalizedContent || !parsedLink || parsedLink.ref !== formal.ref) {
...
export async function updateSessionInDb(
  ...
  const update = async () => {
    ...
    if (p.workspaceKind !== undefined) {
      ...
      // WL-3.6:`'meka'` 是**跨层身份契约**——`workspaceKind === 'meka'` 必须同时带
      // `mekaProjectId` + `mekaRoleId`(反之亦然)。而 `sessionPatchToRow` 不写这两个
      // 身份字段,所以经 `sessions:update` 把普通会话改成 `'meka'` 只会产出一行
      // 「有 meka、无 project/role」的脏身份。**创建是唯一合法写入 `'meka'` 的路径**
      // (create 侧会校验 project+role 并把 Meka 目录登记进 recent_workdirs),
      // 因此这里直接拒绝,而不是放进白名单后靠下方的不可变守卫兜底——那条守卫只在
      // 会话**当前已是** `'meka'` 时生效,拦不住「普通会话 → meka」这个方向。
      if (value === 'meka') {
        throwIpcError(
          'INVALID_PARAMS',
          'Meka workspace identity can only be assigned when the session is created',
        );
      }
    ...
    // Meka 会话的项目/角色身份与工作区是本地事实源(WL-11):一旦落成
    // `workspace_kind = 'meka'`,不得再经 sessions:update 改写工作区身份。
