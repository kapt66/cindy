...
const FROZEN_REPLAY_DEFECT_GUARDS: Record<string, (db: Database.Database) => boolean> = {
  ...
  // 0090 is an append-only schema bridge. Historical synthetic fixtures may
  // intentionally omit the legacy spend tables, or already contain the
  // notification column from a later branch. In those cases the SQL rebuild
  // cannot be applied literally; the runtime companion remains the no-op
  // reconciliation path and the replay must still advance its history.
  '0090_rich_phalanx.sql': (db) => {
    const tableExists = (name: string) =>
      db
        .prepare("SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = ? LIMIT 1")
        .get(name) !== undefined;
    if (!tableExists('daily_model_usage') || !tableExists('daily_spend') || !tableExists('schedules')) {
      return true;
    }
    const columns = db
      .prepare(`PRAGMA table_info('schedules')`)
