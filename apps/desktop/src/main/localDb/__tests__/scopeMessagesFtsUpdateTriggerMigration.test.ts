...
const { default: migration0098 } = (await import(
  '../../../../drizzle/scripts/0098_scope_messages_fts_update_trigger'
)) as { default: { run(db: Database.Database): void } };
...
describe('0098 scope messages_fts update trigger', () => {
  ...
  it('is safe on databases without the messages or messages_fts tables', () => {
    ...
    try {
      expect(() => migration0098.run(empty)).not.toThrow();
    } finally {
      empty.close();
    }
    const withoutFts = createMessagesDb();
    try {
      expect(() => migration0098.run(withoutFts)).not.toThrow();
