...
const { default: migration0097 } = (await import(
  '../../../../drizzle/scripts/0097_repair_custom_provider_timestamps'
)) as { default: { run(db: Database.Database): void } };

describe('0097 repair custom provider timestamps', () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it('surfaces an unexpected schema error instead of marking repair successful', () => {
    ...
    try {
      ...
      `);
      expect(() => migration0097.run(db)).toThrow(/no such column: updated_at/i);
    } finally {
      db.close();
    }
  });
