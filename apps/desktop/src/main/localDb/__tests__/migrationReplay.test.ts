  ...
  it.each(['missing table', 'legacy table', 'existing column'] as const)(
    ...
    (state) => {
      ...
      try {
        ...
        const migration = listMigrations(drizzleDir()).find((item) => item.seq === 107)!;
        ...
        for (let replay = 0; replay < 2; replay += 1) {
          const result = runMigrationReplay(db, { drizzleDir: stagedDir, currentVersion: 106 });
          expect(result.applied.map((item) => item.seq)).toEqual([107]);
          if (state === 'missing table') {
          ...
          expect(db.prepare("SELECT value FROM migration_meta WHERE key = 'schema_version'").pluck().get()).toBe('107');
        }
      } finally {
  ...
  it('rebuilds sessions without losing dependent rows or legacy role-delete triggers', () => {
    ...
    try {
      ...
      db.exec(`${createSessions}
        ...
        CREATE TABLE messages (id TEXT PRIMARY KEY NOT NULL, session_id TEXT NOT NULL REFERENCES sessions(id) ON DELETE CASCADE, role TEXT, content TEXT, created_at INTEGER, rewind_at INTEGER);
        CREATE TABLE right_sidebar_tabs (id TEXT PRIMARY KEY NOT NULL, session_id TEXT NOT NULL, kind TEXT NOT NULL, position INTEGER NOT NULL, is_active INTEGER DEFAULT false NOT NULL, state TEXT DEFAULT '{}' NOT NULL, created_at INTEGER NOT NULL, updated_at INTEGER NOT NULL, FOREIGN KEY (session_id) REFERENCES sessions(id) ON UPDATE no action ON DELETE cascade);
        ...
        INSERT INTO messages (id, session_id) VALUES ('message-1', 'session-1');
