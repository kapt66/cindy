...
describe('Meka formal providers', () => {
  it('accepts only Jira links from the configured project', () => {
    ...
    expect(jiraFormalProvider.parseLink('http://team.atlassian.net/browse/APP-42', configured))
      .toBeNull();
  });
  ...
  it('pins GitLab issue links to the configured HTTPS host and project', () => {
    ...
    expect(
      gitlabFormalProvider.parseLink(
        'https://git.example.com/group/app/-/issues/7',
        configured,
      ),
    ).toEqual({
      ref: '7',
    ...
    expect(
      gitlabFormalProvider.parseLink(
        'https://evil.example.com/group/app/-/issues/7',
        configured,
      ),
    ).toBeNull();
  });
  ...
  it('rejects oversized or malformed frozen content', () => {
    expect(jiraFormalProvider.validateContent({
      title: 'Title',
    ...
    expect(gitlabFormalProvider.validateContent({
      title: 'Title',
      description: 'Description',
  ...
  it('composes deterministic first messages', () => {
    ...
    expect(jiraFormalProvider.composeFirstMessage('https://example/browse/APP-1', content))
      .toBe(jiraFormalProvider.composeFirstMessage('https://example/browse/APP-1', content));
  });
});
