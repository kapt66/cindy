...
export class AgentInputCoordinator {
  ...
  enqueue(
    ...
    if (!schedulerOrigin && item.origin?.kind !== 'orca') {
  ...
  private explainDrainBlockGate(sessionId: string, state: SessionInputState): string | null {
    ...
    if (
      state.recovery?.kind === 'queue-head'
      || (state.recovery && head.origin?.kind !== 'orca')
  ...
  private async drain(sessionId: string, reason: string): Promise<void> {
    ...
    // Orca traffic is system-to-system coordination: draining it past a pending
    // active-turn recovery must not consume the user's Retry entry, so neither
    // the error banner nor the recovery record is cleared for an Orca head.
