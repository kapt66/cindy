...
export async function authorizeMekaHighRiskCallViaDesktop(input: {
  ...
  if (
    !sessionId ||
    !sessionBindings.getSession(sessionId) ||
...
export function registerMakerIpc(maker: Maker, options: RegisterMakerIpcOptions): void {
  ...
  /**
   * 统一的 session bootstrap 序列（7 步，不含 markOrcaRoleIfNeeded）：
   *   1. applyPersistedReviewMode(o)         恢复 Review 隔离模式
   *   2. applyMekaRuntimeConfig(o)           注入 Meka 项目/角色 prompt + MCP runtime
   *   3. applyOrcaInstructions(o)            非 Review 时注入 Orca system prompt
   *   4. applyProjectContextInjection(o)     非 Review 时注入 project-context 知识层
   *   5. validateExtraDirs(o.extraDirs)      过滤非法 extraDirs（仅当非空）
   *   6. maker.createSession(o)              真正创建 session
   *   7. markProjectContextIfNeeded + wireSessionToIpc + markOrcaMcpHydratedIfNeeded
  ...
  async function bootstrapSessionOnce(o: CreateOpts): Promise<BootstrapSessionResult> {
    ...
    const remoteBundle =
      skillSnapshot && hasNativeSkills ? buildMekaRemoteCodexBundle(skillSnapshot) : null;
  ...
  registerMakerSessionCreateHandler(makerSessionRegistry, {
    ...
    reconcileCreateOptsWithDb: (sessionId, opts) =>
      reconcileCreateOptsAgainstDb(sessionId ?? '', opts),
    ...
    sendWorkerReadyMessage: (session) => {
      ...
      const liveSession = maker.getSession(session.id);
    ...
    await reconcileCreateOptsFromPersistedSession({
      ...
      applyRow: async (row) => {
        ...
        if (co.agentKind !== dbMakerKind) {
          log.warn(
            'lazy-create: createOpts agentKind drifted from DB (agent switch); reconciling',
            {
              sessionId,
  ...
  const orcaWorkerCreationService = createOrcaWorkerCreationService({
    ...
    persistMekaWorkerBinding: async ({ sessionId, projectId, roleId }) => {
      ...
      const [current] = await db
        ...
        .where(eq(sessions.id, sessionId))
        .limit(1);
      if (!current || current.workspaceKind !== 'meka') {
      ...
      await db
        .update(sessions)
        .set({ mekaProjectId: projectId, mekaRoleId: roleId, updatedAt: Date.now() })
        .where(eq(sessions.id, sessionId));
    },
