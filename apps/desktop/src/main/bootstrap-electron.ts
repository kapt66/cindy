...
import { registerMekaSkillIpc } from './meka-skills/registerIpc.js';
...
const registerIpcHandlers = () => {
  ...
  ipcMain.handle(MAKER_IPC_INVOKE.XAI_OAUTH_LOGIN, async (event) => {
    ...
    if (owner !== activeOwnerScopeKey() || isAppSessionBoundaryPending()) return { ok: false, reason: 'login_cancelled', authorized: false };
      ...
      if (owner !== activeOwnerScopeKey() || isAppSessionBoundaryPending()) return { ok: false, reason: 'login_cancelled', authorized: false };
      broadcastXaiAuthStateChanged();
      void refreshXaiModelsFromHttp().catch((error) => {
        createLogger('xai-model-refresh').warn('xAI account model discovery after login failed', {
          error: error instanceof Error ? error.message : String(error),
        });
      });
...
app.on('ready', async () => {
  ...
  registerFsBrowseIpc();
  // Remote SSH handlers 同样不依赖 Maker 或 agent binary。必须在首个 renderer
  // 窗口创建前注册，否则启动期 pending-upgrade snapshot 会命中 No handler。
  registerRemoteSshIpc();
  ...
  registerLocalDbIpc({
    ...
    beforeEnsureReady: async (userId) => {
      ...
      // Meka 侧签名已调整为对象（额外用 email 做身份锚点匹配），与上游 user.id 语义等价。
