...
/** Legacy Cindy marker values remain readable; new claims use Cindy Meka values. */
const LEGACY_DEV_NAME = 'CindyDev';
const LEGACY_DEFAULT_NAME = 'Cindy';
...
export function isKeychainIdentityMarkerArtifact(entryName: string): boolean {
  ...
  return entryName.startsWith(`${KEYCHAIN_IDENTITY_MARKER_FILE}.`) && entryName.endsWith('.tmp');
}
...
export type KeychainMarkerRead =
  { kind: 'present'; value: string } | { kind: 'absent' } | { kind: 'unreadable' };
...
function interpretMarker(
  ...
  if (read.kind === 'present') {
    ...
    const complete = read.value.endsWith('\n') ? read.value.slice(0, -1).replace(/\r$/, '') : null;
    const normalizedName =
      complete === LEGACY_DEV_NAME
        ? devName
        : complete === LEGACY_DEFAULT_NAME
          ? defaultName
          : complete;
    if (normalizedName === devName || normalizedName === defaultName) {
