...
/** `AgentDeps.getRemoteAgentFileOps` 的返回形状（远端只读 fs 原语）。 */
type RemoteAgentFileOps = ReturnType<NonNullable<AgentDeps['getRemoteAgentFileOps']>>;
...
export function getMaker(): Maker {
  if (!_maker) {
    ...
    const claudeAgent = new ClaudeCodeAgent({
      ...
      // transport 分类必须先于任何 SSH 专属动作(mcpr-remote-session-routing.md
      // §1/§2):`mcpr:<instance.id>` 是 MCPRouter 隧道的逻辑身份, 永远不在 SSH pool 里,
    ...
    const codexAgent = new CodexAgent({
      ...
      prepareCodexExtraSpawnConfig: async (providers, ctx) => {
        ...
        // Meka: workflow hosts that must not spawn Codex native subagents (see
        // codexNativeSubagentsDisabled) hard-disable them for this isolated host.
        // The flag belongs to the workflow contract, not to the shared smart-routing
        // helper, so it stays at the call site and leaves the pure function upstream-pure.
        const codexSubagentSpawnArgs = disableNativeSubagents
      ...
      getRemoteAgentFileOps: (remoteHostId) => {
        // WL-4.1.2 / mcpr-remote-session-routing.md §1: `mcpr:<instance.id>` 是
        // MCPRouter 隧道的逻辑身份, 不在 SSH pool 里。resume 时 maker-core 会用本
        // 钩子的返回值去读远端 rollout 尾巴 (teammate 指令去重), 提前返回空 reader
        // 表示「远端不可读 ⇒ 重新投递指令」(hasCurrentTeammateInstructions 对
        // 不可读一律返回 false, 绝不跳过), 而不是让它落到 SSH pool 报
        // `remote SSH host "mcpr:<id>" is not connected`, 把 resume 打成启动失败。
        // MCPRouter 的 Codex file-ops 通道不在本轮范围内 —— 缺通道只降级成
        // 「重复投递指令」, 不改变正确性。
        if (classifyRemoteSessionTransport(remoteHostId) === 'mcpr') {
          // 故意返回空 reader。断言是必要的: AgentDeps 把本钩子声明成完整
          // RemoteAgentFileOps, 而空对象只满足下游真正消费的那一项
          // (hasCurrentTeammateInstructions 形参是 Pick<..., 'readFileTail'>,
          // 所以上游调用点写 `?? {}` 能过类型检查, 这里作为返回值不能)。运行时形状
          // 就是「readFileTail 为 undefined」—— 正是「不可读 ⇒ 重新投递指令」的输入。
          return {} as unknown as RemoteAgentFileOps;
        }
        const remoteHost = getRemoteSshPool().get(remoteHostId);
