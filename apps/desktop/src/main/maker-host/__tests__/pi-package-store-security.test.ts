    ...
    const lstatSpy = vi.spyOn(fs, 'lstat').mockImplementation(async (target, options) => {
      ...
      if (!replaced && path.resolve(String(target)) === path.resolve(packageFile)) {
        ...
        try {
          ...
          // Windows without Developer Mode cannot create file symlinks. A hard-link
          // replacement still exercises the same root-identity TOCTOU boundary.
          await fs.link(outsideFile, packageFile);
        }
      }
      return stat;
    });
    ...
    const lstatSpy = vi.spyOn(fs, 'lstat').mockImplementation(async (target, options) => {
      ...
      if (!replaced && path.resolve(String(target)) === path.resolve(packageFile)) {
        ...
        try {
          ...
          const code = (error as NodeJS.ErrnoException).code;
          if (code !== 'EPERM' && code !== 'ENOSYS') throw error;
          // Keep exercising the root-identity race on Windows hosts where file
