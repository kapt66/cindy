...
function fixture(): {
  ...
  // Meka 分歧(有意保留):正式区 userData 目录名由 brand identity 决定(Meka 的
  // cn / global 都是 `CindyMeka`),不是上游的 `CindyGlobal`。写死上游名会让
  // getReleaseCodexAuthPath() 指到不存在的目录,Release 继承相关断言全部假绿/假红。
  const releaseAuth = path.join(
    h.appDataDir,
...
describe('dev 沙箱凭证隔离(XDT_ISOLATED_AUTH)', () => {
  ...
  it('共享 Dev 直接使用 Release 登录态，不重链接或改 binding', async () => {
    ...
    // 共享 Dev = 直接跑在正式区 Release profile 上;Meka 的正式区目录名走 brand identity。
    h.userDataDir = path.join(h.appDataDir, brandUserDataDirName(CURRENT_CINDY_REGION));
    fs.mkdirSync(path.dirname(releaseAuth), { recursive: true });
    fs.writeFileSync(
      releaseAuth,
      JSON.stringify({
