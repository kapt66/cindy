...
describe('remoteCcQueryFactory cleanup wiring', () => {
  ...
  it('threads session vendorOptions into buildCcRemoteHttpMcpServers (worker bootstrap race)', () => {
    ...
    const factoryHeaderEnd = source.indexOf('}) => {', factoryStart);
    expect(factoryHeaderEnd).toBeGreaterThan(factoryStart);
    expect(source.slice(factoryStart, factoryHeaderEnd)).toMatch(
      /startParams,\s*(?:inProcessMcpServers,\s*)?vendorOptions,\s*onApprovalRequest,/,
    );
    const buildArgs = source.indexOf(
...
describe('codex bridge shutdown strip detach gating (Greptile R30 P1)', () => {
  it('detaches idle remote codex sessions only after strip reboots the daemon', () => {
    ...
    const fnEnd = source.indexOf(
      'export async function ensureCodexMcpBridgeStartedForRemote',
      fnStart,
    );
    ...
    const detach = body.indexOf(
      "detachActiveRemoteCodexSessions(hostId, 'bridge-shutdown-strip')",
      gate,
    );
    expect(detach).toBeGreaterThan(gate);
    const detachCalls =
      body.match(/detachActiveRemoteCodexSessions\(hostId, 'bridge-shutdown-strip'\)/g) ?? [];
...
describe('remoteCcQueryFactory drift override (R27 P1)', () => {
  it('does not let the forced-fresh set suppress a detected generation drift', () => {
    ...
    const exemptClose = source.indexOf(
...
describe('remoteCcQueryFactory missing desired generation (Greptile R29 P1)', () => {
  it('forces a clean no-MCP query when injection cannot produce a desired fingerprint', () => {
    ...
    expect(
      source.slice(appliedWrite, source.indexOf('if (appliedFingerprintToWrite)', appliedWrite)),
    ).toContain('ccMissingDesiredStale ? CC_MCP_DISABLED_FINGERPRINT : undefined');
    expect(source).toContain('writeCcAppliedFingerprint(sessionId, appliedFingerprintToWrite)');
  });
});
