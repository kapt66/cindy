...
// Meka: 产品版本（edition）投影。安装包区域固定在 AUTH_REGION，用户可在登录页
// 选择 edition；持久化后供重启恢复与后续登录复用。
...
async function selectLoginRealm(realm: AuthRegion): Promise<AuthFlowState> {
  await loadClientEndpointsForRealm(realm);
  const state = await loadLoginProviders(loginFlowEpoch, realm);
  activeProductEdition = realm;
  return state;
}

async function discoverOrganizationRealm(
  org: string,
  expectedLoginFlowEpoch = loginFlowEpoch,
          ...
          await withCloudOwnerCommit({
            ...
            commit: () =>
              commitWithClearedAccountDeletionReceipt(() => {
                ...
                // Meka edition 投影：兼容会话写入成功后，记录本产品版本，供重启恢复
                // 登录页区域；企业 SSO 跨区发现不改变 edition。
