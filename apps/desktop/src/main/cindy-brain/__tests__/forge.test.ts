...
describe('packGhostDir', () => {
  ...
  it('可将产物直接写入独立输出目录，不依赖跨卷 rename 也不污染开发源码', async () => {
    ...
    try {
      const result = await packGhostDir(dir, { outputDir });

      expect(result.ok, JSON.stringify(result)).toBe(true);
      if (!result.ok) return;
      expect(result.cindyPath).toBe(path.join(outputDir, 'demo-1.0.0.cindy'));
...
describe('FORGE_GUIDE', () => {
  ...
  it('skill 迁移精确映射召回元数据、正文与容器目录', () => {
    const skillSection = FORGE_GUIDE.slice(
      FORGE_GUIDE.indexOf('## 4.17 捆绑 Agent Skills(skill 槽)'),
      FORGE_GUIDE.indexOf('## 4.18'),
    );
    for (const marker of [
      '迁移时按职责映射,不是按篇幅搬运',
  ...
  it('手册覆盖关键章节(身份卡/工具面/管子/聊天卡片/订阅拦截/网络代发/系统提示/沙箱红线/打包)', () => {
    for (const marker of [
      ...
      // 上游 ios-simulator 与 Meka reveal 同时存在，公开卡槽总数为十九个。
      '十九个卡槽',
