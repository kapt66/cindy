...
import { normalizeLegacyMigrationRead, type LegacyMigrationRead } from './legacyMigrationRead.js';
...
export function migrateFiloGoogleAccountsWithResult(
  ...
  const legacy = normalizeLegacyMigrationRead(readLegacyManifest());
  ...
  for (const account of legacy.value.accounts) {
    ...
    const refreshToken = normalizeLegacyMigrationRead(readLegacyRefreshToken(account.id));
