...
export async function packGhostDir(
  ...
  options: {
    iconPng?: Buffer;
    /**
     * Meka 开发目录专用：把每次唯一的临时包直接写进目标卷。避免先在源码目录
     * 生成再用 rename 搬运——Windows 上源码盘与系统临时盘不同时会报 EXDEV，
     * 且失败会在源码目录遗留或覆盖同名包。
     */
    outputDir?: string;
    sessionWorkdir?: string | null;
  ...
  // Forge 打包出口专属安全门(C-4 + #7):source 默认必须在会话 workdir 内;两条越界
  // 通道都必须由 Host 显式放行后才打开——上游的会话权限路径
  // (allowOutsideWorkdir + authorizedDir),以及 Meka 开发目录通道
  // (channel: 'meka' + Host 从当前 data owner 开发注册表下发的 allowedSourceRoots;
  // channel 本身只是业务元数据,不构成授权)。受管根(已装插件 / 批准状态根 /
  ...
  if (!isPathInsideDir(realWorkdir, realSourceDir)) {
    // 通道一(上游)：Host 已按当前会话权限放行跨出 workdir(Full Access 自动放行 /
    // Auto 交当前会话 AI 审阅 / Ask 弹确认卡)。放行的身份必须与真实源目录互相收敛，
    // 否则 authorizedDir 只是"某个别处的目录"。
    const authorizedBySessionGrant =
      options.allowOutsideWorkdir === true &&
      !!options.authorizedDir &&
      isPathInsideDir(options.authorizedDir, realSourceDir) &&
      isPathInsideDir(realSourceDir, options.authorizedDir);
    // 通道二(Meka 开发目录)：源码住在单独 checkout 的 cindy-meka-plugins 仓库。
    // channel 仅是业务元数据，不授予任何越界；真正的授权是 Host 从当前 data owner
    // 的开发注册表下发的规范化源码根(登记根自身不得作为打包目标)。
    ...
    if (!authorizedBySessionGrant && !sourceIsInAllowedMekaRoot) {
      return {
        ok: false,
        errorCode: 'SOURCE_OUTSIDE_WORKDIR',
  ...
  // 默认产物落到规范源码目录(realpath 产物,而非可能是符号链接/别名的入参 `dir`:
  // 产物必须写进真实目录,别名只是通往它的一条路径);
  // Meka 开发目录可显式指定每次唯一的临时根,直接写入目标卷,避免先写源码目录
  // 再 rename 时在 Windows 跨卷触发 EXDEV。
  const outputDir = options.outputDir ? path.resolve(options.outputDir) : realSourceDir;
  const cindyPath = path.join(outputDir, `${built.manifest.id}-${built.manifest.version}.cindy`);
  // 会话放行的授权代次必须在长打包之后、写 .cindy 之前复验:打包期间权限切换
  // (任务 / Plan 代次变化)后，旧的 allow 不可执行。
  const staleBeforeWrite = staleOutsideForgeGrant(options);
  if (staleBeforeWrite) return staleBeforeWrite;
  try {
...
- 用户每次亲选成功,主机自己也会记一笔「亲选目录台账」(每插件最近 8 条)——
  ...
  点选上,你存在 /kv 里的路径不算数。
- reveal 槽只用于把用户亲选结果交给 Host 定位,给路径不扩展文件读写权限。
