...
const INTEROP_SESSION_PREFIX = `${DEEP_LINK_INTEROP_URL_PREFIX}session/`;
...
export function buildSessionDeepLink(sessionId: string, opts?: SessionDeepLinkOptions): string {
  const prefix = opts?.deviceId ? INTEROP_SESSION_PREFIX : SESSION_PREFIX;
  return `${prefix}${encodeURIComponent(sessionId)}${sessionDeepLinkQuery(opts)}`;
}
...
export function buildSessionMessageDeepLink(
  ...
  const prefix = opts?.deviceId ? INTEROP_SESSION_PREFIX : SESSION_PREFIX;
  return `${prefix}${encodeURIComponent(sessionId)}${sessionDeepLinkQuery(opts, messageClientId)}`;
}
