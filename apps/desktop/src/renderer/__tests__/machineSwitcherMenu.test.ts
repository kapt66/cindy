...
describe('远程机器切换入口并入 SidebarTopNav(置顶段上方,固定不滚动)', () => {
  ...
  it('置顶区不受项目 / Agent / 最近活跃筛选影响(归档仍按状态隐藏)', () => {
    ...
    // Meka conversations have their own section and must not be duplicated in the pinned group.
    // 上游本轮把 useProjectGroups 扩成多参数签名,所以入参不再是紧跟在 `(` 之后的
    // `sidebarSessions`;两侧意图合并成同一条断言:该调用必须存在,且入参必须是
    // `nonMekaSidebarSessions`(Meka 会话从普通项目分组/置顶组里排除)。
    expect(sidebarUpperSource).toMatch(
      /const allGroups = useProjectGroups\(\s*nonMekaSidebarSessions\b/,
    );
    expect(sidebarUpperSource).toMatch(
      /const allProjectGroups = useProjectGroups\(\s*nonMekaSidebarSessions\b/,
    );
  });
