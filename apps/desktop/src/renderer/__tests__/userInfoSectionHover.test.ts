...
// Meka 有意的分歧:上游从构建期常量读区域,而 Meka 在 useAuth() 解构里额外取运行期
// edition（可选区域能力，见 UserInfoSection 内的区域注释）。多出的字段让 prettier
// (printWidth=100) 把这次解构拆成多行，所以下面的解构断言在解构块内做格式无关匹配，
// 不绑定上游的单行书写形式。
const useAuthDestructuring = source.match(/const \{[\s\S]*?\} = useAuth\(\)/)?.[0] ?? '';
...
describe('UserInfoSection — Canary avatar badge', () => {
  it('shows only the shield decoration when isCanary is true', () => {
    ...
    expect(useAuthDestructuring).toMatch(
      /\bisCanary\b[\s\S]*?\blistAccounts\b[\s\S]*?\bsyncAccounts\b[\s\S]*?\bswitchAccount\b/,
    );
    expect(source).toContain(
      "if (mode !== 'cloud' || !accountsReadyForOwner || switchableAccounts.length === 0) return null;",
    );
    expect(useAuthDestructuring).toContain('edition = CURRENT_CINDY_REGION');
