      ...
      <>
        <LoginPanel testId="login-panel-identifier">
          ...
          <form onSubmit={submitIdentifier} noValidate>
            ...
            <LoginInput
              // 显式 key:identifier 主视图比 sso-org 子视图多一层区域选择器节点,若
              // 不区分身份,React 会按位置把两个视图的 <input> 认成同一个节点而**复用
              // DOM**(实测 activeElement 从 identifier 输入框原样带到 sso-org 输入框)。
              // 复用的节点不再触发 autoFocus,sso-org 的「多条历史默认折叠」就失效了。
              key="login-identifier-input"
              autoFocus
              disabled={isLoading}
    ...
    <>
      <LoginPanel testId="login-panel-sso-org">
        <form onSubmit={submitSsoOrg} noValidate>
          ...
          <LoginInput
            // 同上:与 identifier 输入框区分身份,保证进入本视图时是全新挂载 ——
            // autoFocus={ssoOrgHistory.length <= 1} 才会按「多条历史默认不聚焦(列表
            // 保持折叠)」生效。
            key="login-sso-org-input"
