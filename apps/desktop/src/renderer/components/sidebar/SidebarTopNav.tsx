...
// 并集:上游本轮新增 botsRow(`Bot` + 返程态 `ArrowLeft`)与 `useLocation`;
// Meka 的 mekaRow 需要 `BriefcaseBusiness`。两侧集合不同,取并集,勿整体取单侧
// (2026-09 上一轮同步就因此丢过 `Bot`,见 meka-whitelist-verification.md WL-2.1)。
import { ArrowLeft, Bot, BriefcaseBusiness, CirclePlus, Plug, Timer } from 'lucide-react';
  ...
  const mekaRow = showScrollable ? (
    ...
    >
      <BriefcaseBusiness
        ...
        className={cn(
          'shrink-0',
          onMekaMatch ? 'text-sidebar-item-active-foreground' : 'text-[var(--sidebar-nav-text)]',
        )}
      />
