...
export function CreateWorkerPopover({
  ...
  const remoteWorkerPermissionModeUnsupported =
    !!deviceId && activeCaps !== null && activeCaps?.supportsOrcaWorkerPermissionMode !== true;
  ...
  const providerFastSupported = useCallback(
    (candidate: string | null, modelId: string): boolean => {
      ...
      if (!sourceId) {
        return deviceId ? !!activeModels.find((m) => m.id === modelId)?.supportsFastMode : false;
      }
  ...
  useEffect(() => {
    if (!supportsWorkerPermissionModeSelection && selectedWorkerPermissionMode !== 'auto') {
