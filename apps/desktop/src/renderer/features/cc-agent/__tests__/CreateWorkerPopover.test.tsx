...
describe('CreateWorkerPopover', () => {
  ...
  it('keeps Codex selectable for a bound MCPRouter target', async () => {
    // Agent 切换在上游已并入统一模型面板(ModelSelector 的 unifiedAgents /
    // onUnifiedSelect),弹窗内不再有独立的 agent tablist —— 见上方
    // "without a competing Harness control"。本用例保留 Meka 语义:绑定 MCPRouter
    // 远程目标后 Codex 仍可选中并创建,不被收窄成 Claude-only。
    mocks.modelsByAgent.codex = [model('gpt-5.5')];
    mocks.capabilitiesByAgent.codex = { availableModels: [{ id: 'gpt-5.5' }] };
    const onCreate = vi.fn();
    const leadSession = {
    ...
    // 统一面板里选 Codex(engine=codex)仍然可用,且不会被目标类型禁用。
    fireEvent.click(screen.getByTestId('pick-codex-config'));
    await waitFor(() =>
      expect(screen.getByTestId('model-selector').textContent).toBe('gpt-5.5'),
    );
