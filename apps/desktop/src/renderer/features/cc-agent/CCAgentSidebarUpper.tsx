...
function ExpandedView({
  ...
  /* ---- Grouping & collapse ---- */
  // WL-3.1:Meka 会话是与项目/对话同级的一级分类,只出现在 Meka 段。普通项目树的
  // **全部**派生(含 allProjectGroups / groupsWithPinnedProjects / projectUniverse)
  // 都必须走 nonMeka*,否则 Meka 会话会重复出现在普通项目分组里。
          ...
          <div hidden={searchActive} className="flex flex-col gap-2">
            {remoteSessionBootstrapFailures.length > 0 && !hasVisibleSidebarContent ? (
              ...
              <>
                 ...
                 * 同样不展示，继续使用本地与已缓存内容即可；远程任务的**部分失败**
                 * 仍保留在列表前，便于用户知道已有内容不是本轮权威结果。
                 */}
                <PinnedSection
