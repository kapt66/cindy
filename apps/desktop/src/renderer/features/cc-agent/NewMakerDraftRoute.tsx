...
export function NewMakerDraftRoute() {
  ...
  const { capabilities, loading: capabilitiesLoading } = useAgentCapabilities(
    capabilityAgentKind,
    effectiveDeviceLinkDeviceId,
  );
  ...
  const { providers: deviceProviders, loading: deviceProvidersLoading } = useDeviceProviders(
    effectiveDeviceLinkDeviceId,
  );
  ...
  const handleSend = useCallback(
    async (
      ...
      if (effectiveCollab.enabled && collabPolicyEligible && policyUnavailable) {
      ...
      if (
        effectiveCollab.enabled &&
        collabPolicyEligible &&
        (policyUnavailable || !policyEnabled)
      ) {
        toast.warning(
          t(
            policyUnavailable
