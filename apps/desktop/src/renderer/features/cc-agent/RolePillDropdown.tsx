...
function WorkerModelLine({ model, effort }: { model: string; effort: string | null }) {
...
function WorkerSummary({
  ...
  return (
    <>
      ...
      {!compact && <WorkerModelLine model={worker.model} effort={worker.effort} />}
      {!compact && directoryLabel ? (
        <div className="mt-0.5 ml-[26px] min-w-0 truncate text-11 leading-snug text-[var(--text-secondary)]">
          {directoryLabel}
        </div>
      ) : null}
    </>
  ...
  return (
    <div className="relative" onMouseEnter={handleMouseEnter} onMouseLeave={handleMouseLeave}>
      ...
      >
        ...
        {resolvedDirectoryLabels.get(worker.workerId) ? (
