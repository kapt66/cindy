...
function holdPackageReview(
  ...
  patchRow(generation, row.pluginId, {
    ...
    expectedManifest: review.manifest,
  });
}
...
async function runApprovalBody(pluginId: string, generation: number): Promise<void> {
  ...
  try {
    ...
    const manifestStillMatches =
      ...
      (row.packageReview
        ? JSON.stringify(row.expectedManifest) === JSON.stringify(row.packageReview.manifest)
    ...
    if (reviewStillValid && row.expectedManifest !== undefined) {
      const didInstall = await installOrHoldForReReview({
        ...
        ...(row.reviewedBaseline !== undefined ? { reviewedBaseline: row.reviewedBaseline } : {}),
        ...(row.packageReview ? { approvedPackageSha256: row.packageReview.packageSha256 } : {}),
      });
