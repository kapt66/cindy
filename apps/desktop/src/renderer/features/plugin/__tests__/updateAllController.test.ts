...
describe('updateAllController', () => {
  // Meka 市场渠道隔离:批次必须始终走启动时绑定的适配器,不能串到 Cindy 渠道。
  it('keeps Meka batch updates on the Meka market adapter', async () => {
    mekaDetailMock.mockResolvedValue(detail({ pluginId: 'meka-plugin-a' }));
    setUpdateAllBatchHooks({
      marketApi: {
        channel: 'meka',
  ...
  // reconcile 也要按渠道判定,否则 Cindy 页面的对账会把 Meka 批次的行改掉。
  it('reconciles only the batch started on the same market channel', async () => {
    mekaDetailMock.mockResolvedValue(detail({ pluginId: 'meka-plugin-a' }));
    setUpdateAllBatchHooks({
      marketApi: {
        channel: 'meka',
