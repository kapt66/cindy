...
interface InstallTargetPickerProps {
  ...
  /** Backward-compatible alias used by local import callers. */
...
export function InstallTargetPicker({
  ...
  const handleInstall = async (installPath?: string) => {
    ...
    setInstalling(true);
    try {
      // Meka 本地导入走自己的 installSkill 通道；市场安装仍带上上游的 catalogScope。
      const res = installSkill
        ? await installSkill({ name: skill.name, installPath })
        : await runAction({ name: skill.name, installPath, catalogScope: skill.catalogScope });
      if (res.success) {
        toast.success(
          t(successToastKey, {
            name: skill.name,
      ...
      if (res.errorCode === 'CONFLICT_USER_OWNED') {
        ...
        const forced = installSkill
          ? await installSkill({ name: skill.name, installPath, force: true })
          : await runAction({
              name: skill.name,
              installPath,
              force: true,
