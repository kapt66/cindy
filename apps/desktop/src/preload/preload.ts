...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  mekaDevPlugins: {
    ...
    package: (id: string): Promise<unknown> => ipcRenderer.invoke('meka-dev-plugins:package', id),
  ...
  mekaSettings: {
    getP4: (): Promise<import('../shared/meka-settings').MekaP4Settings> =>
      ipcRenderer.invoke('meka-settings:get-p4'),
    setP4Root: (directoryPath: string): Promise<import('../shared/meka-settings').MekaP4Settings> =>
      ipcRenderer.invoke('meka-settings:set-p4-root', directoryPath),
    router: {
      ...
      }> => ipcRenderer.invoke('meka-settings:router:list-tools'),
      ...
      listTemplates: (): Promise<import('../shared/meka-router').MekaRouterTemplate[]> =>
        ipcRenderer.invoke('meka-settings:router:list-templates'),
      createInstance: (
        templateId: string,
        name: string,
      ): Promise<import('../shared/meka-router').MekaRouterInstance> =>
        ipcRenderer.invoke('meka-settings:router:create-instance', { templateId, name }),
      getProjectBindings: (projectId: string): Promise<string[]> =>
        ipcRenderer.invoke('meka-settings:project:get-bindings', projectId),
      setProjectBindings: (projectId: string, instanceIds: string[]): Promise<void> =>
        ipcRenderer.invoke('meka-settings:project:set-bindings', { projectId, instanceIds }),
    },
  },
  ...
  localDb: {
    ...
    mekaProjects: {
      list: (): Promise<unknown> => ipcRenderer.invoke('meka-project:list'),
      get: (id: string): Promise<unknown> => ipcRenderer.invoke('meka-project:get', id),
      inspectPath: (projectPath: string): Promise<unknown> =>
        ipcRenderer.invoke('meka-project:inspect-path', projectPath),
    ...
    mekaRoles: {
      list: (projectId: string): Promise<unknown> =>
        ipcRenderer.invoke('meka-role:list', projectId),
      create: (input: unknown): Promise<unknown> => ipcRenderer.invoke('meka-role:create', input),
      update: (input: unknown): Promise<unknown> => ipcRenderer.invoke('meka-role:update', input),
      delete: (id: string): Promise<void> => ipcRenderer.invoke('meka-role:delete', id),
      readManifest: (id: string): Promise<unknown> =>
        ipcRenderer.invoke('meka-role:read-manifest', id),
    },
    mekaProjectMetadata: {
      discover: (projectId: string): Promise<unknown> =>
        ipcRenderer.invoke('meka-project-metadata:discover', projectId),
      list: (projectId: string): Promise<unknown> =>
        ipcRenderer.invoke('meka-project-metadata:list', projectId),
      loadProject: (projectId: string): Promise<unknown> =>
        ipcRenderer.invoke('meka-project:load', projectId),
      saveProject: (input: unknown): Promise<unknown> =>
        ipcRenderer.invoke('meka-project:save', input),
      gitRemote: (projectId: string): Promise<string | null> =>
        ipcRenderer.invoke('meka-project:git-remote', projectId),
    },
    mekaSkillCatalog: {
      list: (): Promise<unknown> => ipcRenderer.invoke('meka-skill-catalog:list'),
    },
    mekaFormal: {
      providerList: (): Promise<string[]> => ipcRenderer.invoke('meka-formal:provider-list'),
      checkAuth: (input: unknown): Promise<unknown> =>
