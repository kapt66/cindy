...
describe('mobile home desktop-first surface', () => {
  ...
  it('renders the remote-access onboarding guide for the no-device empty state', () => {
    ...
    expect(t('deviceLink.connectStep1')).toBe('在电脑上安装并打开 Cindy Meka');
    ...
    // 未来形态预告:云端 Cindy Meka 上线后手机版可脱离电脑直接使用。
    expect(t('deviceLink.cloudTeaserTitle')).toBe('云端 Cindy Meka 筹备中');
